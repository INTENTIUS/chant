...
async function importRsaPrivateKey(pem: string): Promise<webcrypto.CryptoKey> {
  ...
  let keyDer: Uint8Array<ArrayBuffer>;
  if (isPkcs1) {
...
function encodeDerTlv(tag: number, content: Uint8Array): Uint8Array<ArrayBuffer> {
  const len = content.byteLength;
  let lenBytes: Uint8Array<ArrayBuffer>;
  if (len < 0x80) {
...
function concatUint8(...arrays: Uint8Array[]): Uint8Array<ArrayBuffer> {
