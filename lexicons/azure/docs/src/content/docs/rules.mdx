---
title: "All Rules"
description: "Every lint rule and post-synth check provided by the Azure Resource Manager lexicon"
---
...
The Azure Resource Manager lexicon provides **27** rules: 3 lint rules and 24 post-synth checks.
...
| `AZR030` | Resource deployed at an unsupported template scope — split scopes into separate templates |
| `AZR031` | Policy assignment has enforcementMode DoNotEnforce — the guardrail is disabled |
| `AZR032` | Custom policy definition assigned nowhere — a defined but unassigned guardrail enforces nothing |
| `AZR033` | Tenant subscription policy does not block subscriptions from leaving |
