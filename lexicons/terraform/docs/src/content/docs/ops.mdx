...
The push run stops at the apply's gate. It reaches the gate, finds no resolution on the ledger, records the pending fact and ends with status `gated` and exit 3, so the workflow run ends there instead of holding a runner open. `chant approve app-apply approve-app-apply --approver you` records the answer and re-running the workflow applies. A root whose merges should apply unattended sets `gate: "never"`.
