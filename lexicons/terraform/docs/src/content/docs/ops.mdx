...
The runner has no terraform on it, so the install is a `beforeScript` line pinned to a release rather than taken from a package repo: the version that plans is then the version the file names, which matters for a root with a `required_version` floor. `beforeScript` lines are emitted as plain `run:` steps ahead of the invocation, so the Op's first step is never a missing binary.

Permissions are per workflow, from the trigger and the finding mode. The apply half is `contents: read` and nothing else, since it talks to the provider and the state backend rather than to the forge. The plan half is `contents: read` and `pull-requests: write`, which is the whole cost of a comment on the triggering pull request: `findingMode: "comment"` opens no issue, so no `issues: write`, and pushes no branch, so no `contents: write`. The other modes are wider on this trigger, since the trigger adds `pull-requests: write` to any of them: `"issue"` also carries `issues: write`, and `"pull-request"` widens `contents` to write.
