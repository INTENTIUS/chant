...
Permissions are per workflow, from the trigger and the finding mode. The apply half is `contents: read` and nothing else, since it talks to the provider and the state backend rather than to the forge. The plan half gets `pull-requests: write` from the `pull_request` trigger itself, plus whatever its finding mode spends: `findingMode: "issue"` adds `issues: write`, and nothing anywhere adds `contents: write`.
...
**The plan lands as an issue, not as a comment on the pull request.** `reconcilePr`, which is the one place chant shells to `gh`, has three modes: `report` posts nothing, `issue` opens an issue, and `pull-request` regenerates chant TypeScript through `chant import` and opens a PR from it, which is not what a terraform plan wants to say. So the `pull-requests: write` the trigger grants for a comment on the triggering PR goes unspent today. The github lexicon's `PrPlanReport` composite has the sticky-comment mechanism, on the separate `chant lifecycle plan` path rather than on this one.
