...
**Which acceptance suites have actually run.** All four of this lexicon's acceptance blocks skip rather than fail when their binary or emulator is absent, so a green test run says nothing about whether any of them has ever passed. `lexicons/terraform/src/__fixtures__/ACCEPTANCE.md` records the answer per block: what it proves, what gates it, the command to run it, and the last binary and emulator it passed against with the date. The stock apply block passes on any machine with `terraform` or `tofu` and registry access; the live apply pair last passed against a choudoufu built from the v0.13.0 tag with the pinned floci emulator up; the `live-plan` and adopt blocks have never passed, because #894 gates them.
