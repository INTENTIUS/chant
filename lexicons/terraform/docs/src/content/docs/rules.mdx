...
The Terraform lexicon provides **28** rules: 1 lint rule and 27 post-synth checks.
...
| `TF027` | Live root's policy block sets undeclared_untagged = "delete", which deletes resources chant does not own |
| `TF028` | A live root is watched by a TerraformWatchOp built without live: true |
