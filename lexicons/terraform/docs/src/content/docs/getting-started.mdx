...
export const { op } = TerraformApplyOp({
  name: "app-apply",
  root: "app",
  gate: "never", // required for the local executor — see below
});
```

`gate: "never"` matters here specifically: every other gate mode emits an
approval-wait phase, which is durable-runtime work the local executor refuses
to run. With it, `chant run app-apply` executes Init, Plan and Apply against
your terminal, no Temporal server involved. See [Ops](../ops/) for every
`TerraformApplyOp` option, the four activities and builders underneath it, and
the compensation rule for a failed apply.
