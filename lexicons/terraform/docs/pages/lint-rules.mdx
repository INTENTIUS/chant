...
surfaces as a surprise plan diff instead of a version-check failure up to
anything runs.
...
a local path the `.gitignore` files are what separate "committed" from "merely
...
Every `.gitignore` between the file and the scan root is read, the root one
included, so `infra/terraform.tfstate` is not reported when `infra/.gitignore`
ignores it, and auditing from the repository root and from `infra/` agree
([#2528](https://github.com/INTENTIUS/chant/issues/2528); up to `v0.80.0` only
the root `.gitignore` was read).
