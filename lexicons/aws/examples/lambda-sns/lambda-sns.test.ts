...
describe("lambda-sns example", () => {
  ...
  test("build produces valid CloudFormation", async () => {
    ...
    // Composite expands: appTopic + appRole + appFunc + appSubscription + appPermission
    expect(parsed.Resources.appTopic).toBeDefined();
    expect(parsed.Resources.appRole).toBeDefined();
    expect(parsed.Resources.appFunc).toBeDefined();
    expect(parsed.Resources.appSubscription).toBeDefined();
    expect(parsed.Resources.appPermission).toBeDefined();
  });
});
