...
export const awsAuditCatalog: Record<string, RuleMeta> = {
  ...
  WAW044: auditRule("WAW044", "merge-worthy", "guidance", "ALB HTTP listener does not redirect to HTTPS", "Add a redirect DefaultAction (Protocol: HTTPS) to the HTTP listener.", { authority: [AWS_SEC] }),
  ...
  WAW049: auditRule("WAW049", "merge-worthy", "guidance", "Security group allows unrestricted ingress on a non-ALB port", "Restrict the CIDR; only ALB:443 is exempt from this rule.", { authority: [AWS_SEC] }),
