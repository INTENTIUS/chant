...
---
See [Intrinsic Functions](../intrinsics/) for the generated reference table (name, description, output key, whether it's a tagged template, whether folding can fold it). This page is the worked-example companion — one `Fn::` intrinsic function per section, with real usage.
