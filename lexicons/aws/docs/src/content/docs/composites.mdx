...
{/* GENERATED-BY-CHANT-DOCS by `npm run docs -w @intentius/chant-lexicon-aws` — DO NOT EDIT.
    Edit lexicons/aws/docs/pages/composites.mdx instead; changes here are overwritten. */}
...
| `DynamoDBTable` | `table` | Standalone DynamoDB Table (no Lambda glue) for the GSI/TTL/stream/billing-mode configuration CDK apps hand-roll repeatedly. `PAY_PER_REQUEST` by default; `PROVISIONED` accepts table- and per-GSI `provisionedThroughput`, falling back to the table's own capacity. Dedupes `AttributeDefinitions` across the table's keys and every GSI's keys. |
...
| `NlbService` | `nlb`, `targetGroup`, `listener` | Network Load Balancer + target group + listener for TCP/UDP/TLS workloads. No security group is created — NLBs pass through client source IPs, so access control belongs on the targets. Accepts static targets (instance IDs, IPs) or leaves registration to a self-registering service. |
...
| `Ec2InstanceBundle` | `role`, `instanceProfile`, `sg`, `instance` | EC2 instance + security group + instance role/profile, placed into a given VPC/subnet. Inbound rules are opt-in per port (CIDR, IPv6 CIDR, or source security group); no inbound traffic by default. |
