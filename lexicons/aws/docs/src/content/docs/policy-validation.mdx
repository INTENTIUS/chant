...
The recipes above run standalone in any CI job. To make the check a first-class step in a chant [Op](/chant/guide/ops/) — gate-able the same way [`policyGate`](/chant/guide/organizational-policy/#gating-an-apply) gates an apply — use `guardValidate`:
