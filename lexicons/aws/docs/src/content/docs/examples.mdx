...
{/* GENERATED-BY-CHANT-DOCS by `npm run docs -w @intentius/chant-lexicon-aws` — DO NOT EDIT.
    Edit lexicons/aws/docs/pages/examples.mdx instead; changes here are overwritten. */}
...
Produces 4 resources: S3 Bucket (encrypted, public access blocked) + IAM Role (with S3 read policy) + Lambda Function, plus a Bucket Policy in `bucket-policy.ts` that denies requests made without TLS. The `BUCKET_NAME` environment variable is auto-injected.
