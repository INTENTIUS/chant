...
## Allowed values in generated types

Where the CloudFormation Registry schema declares `AllowedValues` for a property, the generated declaration is a union of literals rather than a bare `string` or `number`:

```ts
new LogGroup({ RetentionInDays: 45 });
// Type '45' is not assignable to type '1 | 1096 | 120 | 14 | 150 | 180 | 1827 | ... | 90'
```

The value of that is the error text. It lists the whole valid set, so the compiler answers the question instead of sending you to the console.

The Registry schema is silent on many properties that people write constantly, and it is inconsistent about which: `AWS::SageMaker::NotebookInstance.InstanceType` carried a union while `AWS::EC2::Instance.InstanceType` was a bare `string`. Two sources close that gap at generation time, both of them applied before anything is parsed, and neither of them reachable from `chant build`:

1. **cfn-lint patches.** `src/codegen/patches.ts` fetches the cfn-lint patch set and applies `manual.json`, `smithy.json` and `format.json` to each raw schema. Most of the unions you see beyond the bare Registry come from here.
2. **A curated enum overlay.** `src/codegen/enum-overlay.json` is a checked-in list of `(CloudFormation type, JSON pointer, values)` entries, applied by `src/codegen/enum-overlay.ts`. The first set covers the properties written most often across this repo's examples, docs and composites: Lambda `Runtime`, EC2 `InstanceType`, `Tenancy` and `VolumeType`, ELBv2 `Protocol`, `TargetType` and `Scheme`, SNS `Protocol`, the five DynamoDB table enums, RDS `Engine`, and Application Auto Scaling `ServiceNamespace` and `ScalableDimension`.

An overlay entry mints a named type beside the narrowed property, the same way a `$ref` to an enum definition in the Registry schema does. `AWS::Lambda::Function.Runtime` becomes `Runtime?: Function_Runtime`, and `Function_Runtime` is exported from the lexicon's `types` entry point, so a composite can name the union instead of restating its values.

Two rules govern the overlay:

- The spec wins. An entry whose property already declares `enum`, from the Registry or from a cfn-lint patch, leaves the spec alone and reports itself as redundant so it can be retired. Upstream is refreshed on every `generate` and is the source that keeps tracking AWS.
- An entry that matches nothing is an error. A renamed or removed property fails the generate rather than silently applying curated values to a property that no longer exists.

Each entry records where its values came from, either a botocore service model or an AWS documentation page, and the date they were read. `just refresh-enum-overlay` diffs the botocore-sourced entries against botocore's current `develop` and prints the documentation-sourced ones for a human to re-read. It reaches the network, so it is a manual command and no build step calls it.

Narrowing a property is a breaking change for anyone passing a value the union omits, even though every such value was already rejected at deploy. Overlay changes therefore ship with a release of the lexicon, and the release notes name what newly fails to compile.
...
Declare a condition with the `Condition` declarable and the condition intrinsics (`Equals`, `And`, `Or`, `Not`). The serializer lifts each declared `Condition` into the template's `Conditions` section, the way `Parameter` is lifted into `Parameters`. Reference it with the `If` intrinsic for conditional property values, or with the resource-level `Condition` attribute to gate a whole resource:

{{file:docs-snippets/src/conditions.ts}}

`If`, the resource-level `Condition` attribute, a `stackOutput`'s `condition` option, and `And`/`Or`/`Not` operands all accept either the `Condition` declarable or a condition name string. A template that references a condition it never declares fails the build (WAW069) — CloudFormation would reject it at deploy time. `chant import` regenerates `Conditions` sections from existing templates. Prefer plain TypeScript logic for decisions that can be made at build time; conditions are for decisions the deployment makes from parameter values.
