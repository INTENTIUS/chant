 ...
 *                          `runComponentDeploy`'s local semantics exactly. Each
 *                          executed step's own `run()` output is carried
 *                          alongside it (`executed`, below) and passed to
 *                          `rollbackCapabilityStep` as the durable identity
 *                          channel `Capability.rollback`'s third parameter
 *                          describes (#1944) — the local driver's in-process
 *                          object identity (a `WeakMap` keyed by the exact
 *                          `input` a capability's `run()` was called with, see
 *                          e.g. `@intentius/chant/components/verbs/run-agent`)
 *                          never survives across this workflow's separate
 *                          `runCapabilityStep`/`rollbackCapabilityStep`
 *                          Activity invocations, so a capability that needs to
 *                          recover state from its own prior `run()` call has
 *                          no other channel here. A rollback failure is also
 *                          no longer silently swallowed: it's logged
 *                          (`log.error`) and recorded into a `RollbackFailed`
 *                          search attribute so it's visible on the completed
 *                          (failed) workflow even though the unwind stays
...
function generateWorkflow(component: DriverComponent, options: SerializeComponentOptions): string {
  ...
  const lines: string[] = [
    ...
    "import { proxyActivities, condition, defineSignal, setHandler, upsertSearchAttributes, log, rootCause } from '@temporalio/workflow';",
  ...
  lines.push("  // `output` is each step's own run() result, carried alongside it so saga");
  lines.push("  // rollback can pass it to rollbackCapabilityStep as the durable identity");
  lines.push("  // channel (#1944) — see this file's module doc comment, \"onFailure\".");
  lines.push("  const executed: Array<{ step: Record<string, unknown>; phaseName: string; output: unknown }> = [];");
  lines.push("");
  ...
  const renderEntry = (entry: DriverStep | DriverPhase, phaseName: string, ind: string): string => {
    ...
    lines.push(`${ind}executed.push({ step: ${stepVar}, phaseName: ${JSON.stringify(phaseName)}, output: ${outVar} });`);
    return outVar;
  };
  ...
  const renderRollback = (phases: DriverPhase[] | undefined, indent: string) => {
    ...
    // `output: __e.output` is the durable identity channel (#1944) — see this
    // file's module doc comment, "onFailure". A rollback failure is no longer
    // silently swallowed: it's logged AND recorded into `__rollbackFailures`,
    // surfaced via a `RollbackFailed` search attribute below — visible on the
    // completed (failed) workflow even though the unwind itself still never
    // masks the original error by throwing.
    lines.push(`${indent}const __rollbackFailures: Array<{ kind: string; phase: string; error: string }> = [];`);
    lines.push(`${indent}for (const __e of [...executed].reverse()) {`);
    lines.push(`${indent}  try {`);
    lines.push(
      `${indent}    await rollbackCapabilityStep({ step: __e.step, phase: __e.phaseName, component: ${JSON.stringify(component.name)}, env: __env, vars: __vars, phaseOutputs, componentOutputs, output: __e.output });`,
    );
    lines.push(`${indent}  } catch (__rollbackErr) {`);
    lines.push(`${indent}    // rootCause unwraps the ActivityFailure/ApplicationFailure wrapper Temporal`);
    lines.push(`${indent}    // puts around a failed activity, so this is the real error message, not`);
    lines.push(`${indent}    // a generic "Activity task failed".`);
    lines.push(`${indent}    const __rollbackErrMsg = rootCause(__rollbackErr) ?? String(__rollbackErr);`);
    lines.push(`${indent}    log.error(\`rollback failed: component="${component.name}" phase="\${__e.phaseName}" kind="\${__e.step.kind}": \${__rollbackErrMsg}\`);`);
    lines.push(`${indent}    __rollbackFailures.push({ kind: __e.step.kind as string, phase: __e.phaseName, error: __rollbackErrMsg });`);
    lines.push(`${indent}    // best-effort unwind — never mask the original error`);
    lines.push(`${indent}  }`);
    lines.push(`${indent}}`);
    lines.push(`${indent}if (__rollbackFailures.length > 0) {`);
    lines.push(`${indent}  upsertSearchAttributes({ RollbackFailed: __rollbackFailures.map((f) => JSON.stringify(f)) });`);
    lines.push(`${indent}}`);
