...
function generateWorkflow(config: OpConfig): string {
  ...
  const lines: string[] = [
    ...
    `import { proxyActivities, condition, defineSignal${allGateSteps.length > 0 ? ", defineQuery" : ""}, setHandler, upsertSearchAttributes${hasEffects ? ", log" : ""} } from '@temporalio/workflow';`,
  ...
  if (allGateSteps.length > 0) {
    ...
    // Queryable gate state (chant #1676): "which gate, if any, is this
    // workflow waiting on, and since when" — a renderer's alternative to
    // inferring it from RUNNING plus a stalled activity count. One shared
    // query answers for the whole Op, not per-gate, because only one gate
    // step can be awaited at a time (gate steps run in program order even
    // inside a "parallel" phase — only its activity runs are concurrent).
    lines.push(`const gateStateQuery = defineQuery<{ signalName: string; description?: string; since: string } | null>(${JSON.stringify("gateState")});`);
    lines.push("");
  }
  ...
  if (allGateSteps.length > 0) {
    lines.push(`  let __gateState: { signalName: string; description?: string; since: string } | null = null;`);
    lines.push(`  setHandler(gateStateQuery, () => __gateState);`);
    lines.push("");
  }
  ...
  const emitGate = (gateStep: GateStep, out: string[], indent: string) => {
    ...
    out.push(
      `${indent}__gateState = { signalName: ${JSON.stringify(gateStep.signalName)}, ` +
        `${gateStep.description ? `description: ${JSON.stringify(gateStep.description)}, ` : ""}` +
