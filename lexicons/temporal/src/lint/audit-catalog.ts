 ...
 * #2114 sub-issue 6) and dropped out of this lexicon's own catalog with them
 * — they're op-model checks now registered core-wide, not Temporal-specific.
 *
 * Three of the four remaining checks read the chant model (`ctx.entities`)
 * rather than the emitted output, so they carry `yamlBased: false` and cannot
 * fire on an audit of standalone Temporal config; TMP010 reads `ctx.outputs`
 * and can. `auditRule()` hardcodes `yamlBased: true`, so these are
