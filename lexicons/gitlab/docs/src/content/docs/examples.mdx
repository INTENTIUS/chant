...
Use `BunPipeline` for projects that use the [Bun](https://bun.sh) runtime. It is a preset
of `NodePipeline` with Bun-specific defaults: `bun install --frozen-lockfile`, cache under
`.bun/install/cache` keyed on `bun.lock`, and `bun run` as the script prefix.

```typescript title="pipeline.ts"
import { BunPipeline } from "@intentius/chant-lexicon-gitlab";

export const app = BunPipeline({
  buildScript: "build",
  testScript: "test",
});
```

To keep most `NodePipeline` options while switching the package manager:
