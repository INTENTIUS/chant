 ...
 * Reads a .gitlab-ci.yml file, parses it, generates TS, dynamically imports it,
 ...
 *
 * Usage: npx tsx scripts/full-roundtrip-helper.ts [--skip-serialize] <pipeline-file>
 */
...
if (import.meta.main) {
  ...
  if (!file) {
    console.error("Usage: npx tsx full-roundtrip-helper.ts [--skip-serialize] <pipeline-file>");
    process.exit(1);
  }
 ...
 * compare resource-level structure (count + entity types match).
 */
async function verifySerialize(
  ...
  const { gitlabSerializer } = await import("../src/serializer");
  ...
  const yamlOutputRaw = gitlabSerializer.serialize(entities);
  ...
  if (process.env.VERBOSE) {
    console.log(
      `OK (full roundtrip): ${entities.size} declarable(s), ${roundtrippedKinds.length} resource(s) match`,
    );
  }
