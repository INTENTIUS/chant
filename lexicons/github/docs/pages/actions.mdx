...
Wraps `actions/checkout@v7`. Clones the repository.
...
Checkout({ pin: "sha" }).step                  // Pinned to a commit SHA
```

**Props:** `ref?`, `repository?`, `fetchDepth?`, `token?`, `submodules?`, `sshKey?`, `pin?`

`pin: "sha"` emits `actions/checkout@<sha> # v7.0.1`, the commit the lexicon's pin table records for the release, with the version as a YAML comment. GHA021, GHA029 and GHA059 accept that form. The table is `ACTION_PINS`, exported from the package, and the same `pin` prop is on `SetupNode`.

### SetupNode

Wraps `actions/setup-node@v7`. Installs Node.js with optional dependency caching.
...
SetupNode({ nodeVersion: "22", pin: "sha" }).step  // Pinned to a commit SHA
```

**Props:** `nodeVersion?`, `cache?` (`"npm"` | `"pnpm"` | `"yarn"`), `registryUrl?`, `pin?`
