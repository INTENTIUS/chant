...
Flags `actions/checkout` referenced by a tag (e.g. `@v4`) instead of a pinned commit SHA. The narrower precursor to GHA029, kept because checkout is the most common unpinned action. A trailing version comment (`actions/checkout@<sha> # v7.0.1`) is allowed. The warning names the exact pinned `uses:` value, which `Checkout({ pin: "sha" })` emits.
