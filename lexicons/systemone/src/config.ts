/**
 * The `systemone` namespace in `chant.config.ts`: the backends a point's
 * model decider names.
 *
 * ```ts
 * import type { ChantConfig } from "@intentius/chant/config";
 * import "@intentius/chant-lexicon-systemone";   // brings the `systemone` key into ChantConfig
 *
 * export default {
 *   lexicons: ["systemone"],
 *   systemone: {
 ...
 * key here. A backend is a profile, as `fountain.profiles` is: an endpoint
 * that speaks the `POST /v1/systemone` wire format (TypeSafe's, or any server
 * that implements it, #2491) and where its key comes from.
 *
...
export type SystemoneBackend = z.infer<typeof backendSchema>;
 ...
 * (#1344): without it this line fails to compile.
 */
export type SystemoneConfigNamespace = NonNullable<ChantConfig["systemone"]>;
