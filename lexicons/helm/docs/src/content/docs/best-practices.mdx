...
`helmInstall` can also deploy a recorded render's exact bytes. Pass `contentDigest` instead of a chart reference and the activity takes the pinned path. It loads the render from the render store, verifies the stored bytes still hash to the digest, routes them by their `# Source:` origin, and installs them as a structure-preserving wrapper chart. Helm renders nothing at deploy time.
