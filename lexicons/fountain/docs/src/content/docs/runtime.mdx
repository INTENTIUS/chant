...
The first records the resolution. The second posts `chant run <op>` back onto the steward's thread, carrying `--approver` and `--url` along so the thread names who resolved the gate. The sandbox re-runs the op and walks through the gate, reading the resolution back off the ledger.

`--durable-requests` would instead answer fountain's own pending permission card. It is refused today by name: the request-answer path it needs is [fountain#1635](https://github.com/managoat/fountain/issues/1635), which fountain v0.21.0 carries and the fountain runtime does not use yet (chant#2391). Drop the flag and use the two commands above.
