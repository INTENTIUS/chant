...
`chant workspace status <env> --json` lists each member's stewards: the name, the file that declares it, its form in `<env>` and its declared forms, the local lease (holder, expiry, and whether it is live), and each Op with its schedule, its last run from the run ledger and the work lease its turn holds. See [workspace status](/chant/cli/workspace-status/).
