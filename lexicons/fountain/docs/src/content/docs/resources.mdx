...
also made `model` optional. FTN023 keeps the two together, and FTN016 rejects a
`model` on an acp agent, because the model is whatever the command on the other
end of the protocol chooses.
