...
export const flywaySerializer: Serializer = {
  ...
  serialize(entities: Map<string, Declarable>, _outputs?: LexiconOutput[]): string | SerializerResult {
    ...
    // databaseType is chant-internal metadata, not a valid Flyway TOML parameter
    const INTERNAL_KEYS = new Set(["databaseType"]);
    ...
    for (const [, entity] of projects) {
      ...
      if (props) {
        ...
        for (const [key, val] of Object.entries(props)) {
          if (val !== undefined && !INTERNAL_KEYS.has(key)) doc[key] = val;
        }
      }
    }
    ...
    for (const [, entity] of configs) {
      ...
      if (props && Object.keys(props).length > 0) {
        ...
        for (const [key, val] of Object.entries(props)) {
          if (!INTERNAL_KEYS.has(key)) filtered[key] = val;
        }
        if (Object.keys(filtered).length > 0) {
          doc.flyway = { ...(doc.flyway as Record<string, unknown> ?? {}), ...filtered };
        }
      }
    }
    ...
    return emitTOML(doc, {
      ...
      keyOrder: ["id", "name", "flyway", "environments", "flywayDesktop", "redgateCompare"],
    });
  },
};
