/**
 * Helper script for import-k8s-samples.sh
 * Reads a K8s manifest file, parses it, generates TS output,
 ...
 *
 * Usage: bun run scripts/roundtrip-helper.ts <manifest-file>
 */
...
if (import.meta.main) {
  ...
  if (!file) {
    console.error("Usage: bun run roundtrip-helper.ts <manifest-file>");
    process.exit(1);
  }
