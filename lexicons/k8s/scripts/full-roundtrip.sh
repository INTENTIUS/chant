...
# Full roundtrip test: YAML → parse → generate → import → serialize → compare
# Clones kubernetes/examples and runs the full pipeline on each manifest.
...
usage() {
  echo "Usage: $0 [--skip-clone] [--skip-serialize] [--verbose] [--manifest NAME]"
  echo ""
  echo "  --skip-clone      Skip git clone if repo already exists"
  echo "  --skip-serialize  Skip serialize phase (parse+generate only)"
...
if [[ "$SKIP_SERIALIZE" == "true" ]]; then
  echo "Running roundtrip tests (parse+generate only)..."
else
  echo "Running full roundtrip tests (parse → generate → import → serialize)..."
fi
echo ""
...
while IFS= read -r manifest; do
  ...
  export VERBOSE
  if bun run "$HELPER" ${HELPER_ARGS[@]+"${HELPER_ARGS[@]}"} "$manifest" 2>/dev/null; then
    ((pass++)) || true
    if [[ "$VERBOSE" == "true" ]]; then
      echo "PASS: $rel"
    fi
