...
description: "Argo CD support in the k8s lexicon — the argoproj.io CRDs, ArgoAppFor / ArgoAppSetForRegions composites, cluster registration, the ARGO00x rules, and how a deploy splits between Argo, an Op and CI."
diataxis: reference
---

{/* GENERATED-BY-CHANT-DOCS by `npm run docs -w @intentius/chant-lexicon-k8s` — DO NOT EDIT.
    Edit lexicons/k8s/docs/pages/argo-composites.mdx instead; changes here are overwritten. */}
...
## The three-layer model: Argo, an Op and CI
...
| **A chant Op** | Procedural steps with ordering, human gates, one-shot RPCs | the step is a *procedure*, not a state — DNS delegation, cert generation, `db init` |
| **CI** | One-shot, fire-and-forget apply | a simple pipeline with no reconciliation or long-running orchestration need |

Rule of thumb: **if it's declarative and converges, let Argo reconcile it; if it's a procedure with ordering or gates, run it as a chant Op.** Prefer Argo CD over Argo *Workflows* — the procedural layer stays an Op. See the [CockroachDB multi-region tutorial](/chant/tutorials/cockroachdb-multi-region/) for a deployment that uses both.
...
- [Flux Composites](../flux-composites/) — the Flux counterpart: `FluxGitSource` + `FluxAppFor`, the FLUX00x rules, and the `flux-reconcile` deploy step.
- The `chant-k8s-argo` [skill](../skills/) — agent guidance for these patterns.
- [CockroachDB multi-region tutorial](/chant/tutorials/cockroachdb-multi-region/) — Argo and a chant Op together.
