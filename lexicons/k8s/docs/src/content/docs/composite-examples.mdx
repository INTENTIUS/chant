...
Namespaces with `pod-security.kubernetes.io/enforce: restricted` require every container to declare four security fields. All user-facing composites accept a `securityContext` prop that passes through to the container:
...
Infrastructure composites — `ExternalDnsAgent`, `FluentBitAgent`, `AdotCollector`, and `MetricsServer` — apply PSS-safe defaults (`runAsNonRoot`, `readOnlyRootFilesystem`, `allowPrivilegeEscalation: false`) automatically.
