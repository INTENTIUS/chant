...
{/* GENERATED-BY-CHANT-DOCS by `npm run docs -w @intentius/chant-lexicon-k8s` — DO NOT EDIT.
    Edit lexicons/k8s/docs/pages/composite-examples.mdx instead; changes here are overwritten. */}
...
Not yet documented on this page or its vendor-specific neighbors (chant #1723's audit of
`lexicons/k8s/src/composites/index.ts` against the docs):

- **Generic:** `StatefulApp`, `BatchJob`, `SecureIngress`, `ConfiguredApp`, `SidecarApp`,
  `MonitoredService`, `NetworkIsolatedApp`, `MetricsServer`
- **CockroachDB:** `CockroachDbCluster` — the self-managed, multi-region StatefulSet pattern
  that [`CockroachDbRegionStack`](../gke-composites/#cockroachdbregionstack) builds on; see the
  [CockroachDB Multi-Region tutorial](/chant/tutorials/cockroachdb-multi-region/) in the
  meantime
- **Ray:** `RayCluster`, `RayJob`, `RayService` — see the
  [Ray + KubeRay tutorial](/chant/tutorials/ray-kuberay-gke/) in the meantime
- **KServe:** `InferenceService` — spec-true predictor (model ref + `ServingRuntime`
  name) with autoscaling bounds and canary traffic split; pairs with a
  `ServingRuntime` (e.g. vLLM) and a resolved model `storageUri`
