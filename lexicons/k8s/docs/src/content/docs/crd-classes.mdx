...
Every field there is policy: where the secret comes from, who may fetch it, what happens to the materialized Secret when the CR is deleted. None of it is a value — which is exactly why this kind is expressible in chant. Synthesis resolves nothing, so the manifest stays safe to commit. See [Where Values Come From](/chant/concepts/where-values-come-from/).

<Aside type="caution">
`creationPolicy` decides what deletion means. `Orphan` leaves the materialized Secret behind when the CR goes away; `Owner` takes it with it, so deleting the CR takes down anything reading that Secret.
</Aside>
...
<Aside type="note">
Flux is a **source-as-truth** reconciler — its controllers converge the cluster to what's in git. That is a different lifecycle model from chant's own observational one; chant only **emits** the typed Flux resources. The role split mirrors [Argo CD](/chant/lexicons/k8s/argo-composites/#the-three-layer-model-argo-an-op-and-ci): reach for Flux (or Argo) when the desired state lives in git and converges; reach for an [Op](/chant/guide/ops/) when the step is a procedure with ordering or gates.
</Aside>

The `FluxGitSource` and `FluxAppFor` composites collapse the `GitRepository` + `Kustomization` pair above into two calls with estate-tested defaults, and the FLUX001–003 [lint rules](/chant/lexicons/k8s/lint-rules/#flux) validate the source pin, `sourceRef`, and `dependsOn` edges.
...
What you give up is the whole point of a generated class: no typed constructor, no spec validation at lint time, no LSP hover. Prefer generating the CRD. Two callers use this deliberately — a [kustomize build root](/chant/lexicons/k8s/importing-yaml/), whose output is already finished documents, and `chant carve emit` adopting a Terraform `kubernetes_manifest`, whose kind is only known once the body is read.
