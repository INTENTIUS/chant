...
| Rule | Description |
|------|-------------|
| WGC111 | Dangling resource reference — `resourceRef.name` points to a name not defined in the output |
| WGC112 | Missing or invalid `apiVersion` — must match `<service>.cnrm.cloud.google.com/v<version>` |
| WGC113 | Alpha API version warning — prefer `v1beta1` or `v1` over unstable `alpha` versions |
