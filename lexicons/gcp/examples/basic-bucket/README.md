...
A GCS bucket with uniform bucket-level access, versioning, and lifecycle rules — built using the `GcsBucket` composite.
...
>
> ```
> Deploy the basic-bucket example to my GCP project.
> ```
...
```bash
chant spell cast gcp-basic-bucket
```
...
|------|-----------|-----------|
| `src/infra.ts` | `GcsBucket` | StorageBucket |
