...
A Cloud Function with Pub/Sub trigger, dead-letter queue, and source bucket -- built using the `CloudFunctionWithTrigger` and `PubSubPipeline` composites.
...
>
> ```
> Deploy the cloud-function example to my GCP project.
> ```
...
```bash
chant spell cast gcp-cloud-function
```
...
|------|-----------|-----------|
| `src/infra.ts` | `CloudFunctionWithTrigger`, `PubSubPipeline` | CloudFunctionsFunction, StorageBucket, PubSubTopic, PubSubSubscription |
...
- [cloud-sql](../cloud-sql/) -- Cloud SQL PostgreSQL instance with database and user
