...
Coverage under `--sandbox` is measured separately, because the #1093 refusal can only demote: across the same 101 directories it costs **2** files and **zero** fully-folded entries, so `--sandbox` folds the same 55. It cost 3 before chant #1023, which interprets the one project-owned composite factory in the corpus rather than calling it; the 2 that remain are a different refusal — an imported project value read by name through an intrinsic, not a factory. Real chant source imports its constructors and composites from lexicon packages; a project-owned factory is rare, and where its body leaves the interpretable subset the file simply runs in the child instead. The numbers are reported by `just sandbox-differential`.

`just fold-differential` reports the plain-`--fold` half of the same accounting — how many composite factories a build actually invoked in this process, how many of those were project-owned, and how many bodies were interpreted instead. Across the corpus that is 234 invocations, **1** of them project-owned (a plain wrapper function, which is not a registered composite and so is deliberately outside #1023's subset), and 1 interpretation.
