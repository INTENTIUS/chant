...
Most commands evaluate `chant.config.ts` before they do anything else, in the sandboxed child when `--sandbox` is given and in the CLI's process otherwise. Three commands never evaluate it, with or without `--sandbox`:
...
`chant import --agents` does load a lexicon declared by path from its module, which is project code, the same way `chant build` does.
