...
Fold currently covers 32 of the corpus's 98 example directories in full; the other 66 have at least one file that falls back to run. `--sandbox` isolates that 66 today, at the same coverage fold has now — increasing fold coverage (tracked separately; #1044 landed registered call-form intrinsics, #1063 is the largest remaining cause) is an optimization on top of that, not a precondition for it.
