...
| Ambient environment | The child is spawned with a scrubbed `env`, not a permission flag | `process.env` shows only `PATH` (plus `CHANT_ENV` in the config child — see below); nothing else the parent process had is visible |
...
With `chant build --sandbox`, no file the project authored is executed in the CLI's own process while chant reads the project's configuration and discovers its resources — not its source files, folded or not, and not its `chant.config.ts`. The only code that runs there is chant's own and the packages of the lexicons this build loaded.
...
`examples/sandbox-execution-boundary.test.ts` asserts this over the whole corpus. Every one of the 101 example directories has its config loaded and is then built with `{ fold: true, sandbox: true }`, while the only two places project code can execute in this process are recorded — `importModule` (`packages/core/src/discovery/import.ts`) and `importConfigModule`/`requireConfigModule` (`packages/core/src/config-import.ts`). No path inside the project's own source directory may appear in the first, and the second must not be called at all.

Two residuals remain, both pre-existing and neither closed by this flag:
...
- **`lint.policies` and `lint.plugins` are project-authored modules chant loads and calls.** They are *declared* in the config as paths, so the config crossing the boundary as data does not make them cross with it — `chant build` imports each policy module and calls its `check` function against the built resources, in this process, after discovery is over. That is the point of them: a policy is a callback over the result, not data. Sandboxing them needs a different design than this one (the function has to run somewhere it can see the resolved resources), and is not attempted here.
...
Fold covers 55 of the corpus's 101 example directories in full; the other 46 have at least one file that falls back to run. `--sandbox` isolates that remainder — increasing fold coverage (tracked separately) is an optimization on top of that, not a precondition for it.

Coverage under `--sandbox` is measured separately, because the #1093 refusal can only demote: across the same 101 directories it costs 3 files and **zero** fully-folded entries, so `--sandbox` folds the same 55. Real chant source imports its constructors and composites from lexicon packages; a project-owned factory is rare, and where it exists the file simply runs in the child instead. The numbers are reported by `just sandbox-differential`.
