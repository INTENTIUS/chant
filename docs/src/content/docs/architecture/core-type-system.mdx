...
`expect` composes independently-optional clauses: `noop: true`; exact `create`/`update`/`delete` counts; `deletes: [{ name, ownership }]` for named, ownership-checked deletes; `unobserved: "refuse" | { allow: [names] }` for the plan's unobserved holes; and `cost: { maxPerHour, currency, entity? }`, a bound on one predicted rate read from the fixture's `behaviour` block. At least one clause is required.
