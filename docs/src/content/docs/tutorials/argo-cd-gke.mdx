...
The smallest example of the **three-layer split**: Chant authors typed manifests, Argo CD reconciles them, and an Op on a steward (not needed here) drives anything procedural. A single GKE cluster runs Argo CD, and one Chant-authored workload is deployed through a `K8s::Argo::Application` declared in TypeScript with [`ArgoAppFor`](/chant/lexicons/k8s/argo-composites/).
...
- Gate procedural steps on Argo finishing with the k8s lexicon's `waitForArgoSync` activity — see the [CockroachDB multi-region tutorial](/chant/tutorials/cockroachdb-multi-region/), which uses Argo for the apply layer and a steward for the procedural one.
- Running Flux instead of Argo? The same split, on a self-hosted estate: [Flux CD Self-Hosted](/chant/tutorials/flux-apps/).
