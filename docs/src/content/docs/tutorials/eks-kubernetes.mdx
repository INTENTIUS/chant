...
| `src/infra/params.ts` | CF parameters: environment, domain, cert ARN |
...
The `load-outputs` target queries `aws cloudformation describe-stacks`, extracts the IAM role ARNs, and writes them to `.env`. Bun auto-loads `.env` at runtime, so the next K8s build uses real ARNs instead of placeholders.
