...
| `src/infra/params.ts` | CF parameters: environment, domain, cert ARN, public access CIDR |
...
# Deploy the CloudFormation stack (32 resources)
# Pass your domain, ACM cert ARN, and optionally restrict API endpoint access:
...
The stack creates the VPC, EKS cluster, managed node group, OIDC provider, 7 IAM roles, and 5 EKS add-ons. The ALB controller add-on installs automatically — no Helm charts or CRDs to manage separately.

The `cidr=` parameter restricts EKS API server public access to your IP. Omit it to allow access from any IP (0.0.0.0/0). In production, always restrict to your corporate CIDR or use private-only endpoint access.
...
- **K8s-only changes** (app config, scaling, new workloads): edit source, run `just build-k8s && just apply`. No infra redeploy needed.
- **Infrastructure changes** (node count, new IAM roles, new addons): edit source, run `just build && just deploy-infra`. Then run `just load-outputs` if outputs changed, followed by `just build-k8s && just apply`.
