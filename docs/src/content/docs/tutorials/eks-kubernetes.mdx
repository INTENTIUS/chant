...
EKS control plane ~$73/mo, NAT gateway ~$32/mo, ALB ~$16/mo, 3× t3.medium nodes ~$100/mo. **Total ~$221/mo** (us-east-1 list price, checked 2026-08 against chant 0.46.0; AWS list prices change and vary by region — re-price before relying on this number). Teardown after testing.
...
```typescript
// src/infra/irsa.ts — the IAM role for the app workload
//
// Both condition keys carry the OIDC issuer, and CloudFormation substitutes
// into a string, so the trust policy is one Fn::Sub over the whole JSON
// document. It is written as string literals rather than built by a call, so
// the file stays statically evaluable.
const appTrustPolicy = {
  "Fn::Sub": [
    trustPolicyHead + "microservice:microservice-app-sa" + trustPolicyTail,
    { OidcArn: oidcProvider.Arn, OidcIssuer: oidcIssuer },
  ],
};
...
`trustPolicyHead` ends at `"system:serviceaccount:` and `trustPolicyTail` picks
up after the namespace and service account name, so each role differs by one
literal.
