...
| Op | Point |
|----|-------|
| `deploy` | the happy path — checkpoint, build, deploy, verify, tear down |
| `deploy-guarded` | the rollback climax — a risky follow-up step fails and the Sprite rewinds |
...
`deploy` exits `ok`. The phases run with no raw shell: boot the emulators, create
...
`deploy-guarded` is the same setup, then a risky follow-up step (`./risky.sh`, run
