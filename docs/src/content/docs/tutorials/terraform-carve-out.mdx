...
- **`carve emit` and `carve apply` — AWS.** Emit produces native chant source
  for the AWS lexicon and covers the common carve targets: S3, IAM (role /
  policy / instance profile), DynamoDB, SNS, SQS, Lambda, KMS, Secrets Manager,
  SSM parameters, ECR, CloudWatch (log groups / alarms), Route 53, EFS, and the
  core EC2 networking resources (VPC, subnet, security group, route table,
  internet/NAT gateway, EIP). Advise and emit cover exactly the same AWS types,
  so anything advise ranks, emit can produce. A type outside that set reports
  the supported list. Other clouds are not carved yet.
