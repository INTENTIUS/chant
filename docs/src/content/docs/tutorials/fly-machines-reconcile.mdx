...
`src/infra.ts` is one app and two machines — `web` and `worker`:
...
const web = new Machine({ name: "web", region: "iad", config: new MachineConfig({ image: "flyio/hellofly:latest", guest }) });
...
on its own — the ownership marker the owned-only prune reads back.
