...
| Phased deployment | 10-stage canary-first pipeline |
...
- The 10-stage canary-first pipeline and why each stage exists
...
`npm run build` writes the four production outputs from `src/`:
...
It then builds `k3d/` for the local smoke test below: `k3d.yaml` (K8s) and
`k3d-cluster.yaml`, from the k3d lexicon, the fifth this example declares.
...
The `register-runners` pipeline job runs **after all cells are deployed** — it's stage 7 of 10. The routable token format (`glrt-t{cellId}_{random}`) encodes the cell ID, so the Cell Router can route CI job requests to the right cell's Workhorse without a topology lookup.

### The 10-stage canary-first pipeline

```
1. infra              →  kubectl apply config.yaml  (Cloud SQL, Redis, GCS)
2. system             →  kubectl apply k8s.yaml     (NGINX, cell-router, cert-manager, ESO)
3. build-helm-values  →  npm run build:helm         (values-<cell>.yaml, kept as artifacts)
4. validate           →  helm diff per cell         (dry-run preview)
5. deploy-canary      →  helm install cell-alpha    (wait for rollout)
6. deploy-remaining   →  helm install cell-beta, cell-gamma, ...  (parallel, depends on canary)
7. register-runners   →  GitLab API per cell, store token Secret, restart runner
8. smoke-test         →  scripts/e2e-test.sh        (real HTTP requests, routing assertions)
9. backup             →  backup-utility per cell    (scheduled job: GCS snapshots)
10. migrate-org       →  manual trigger only        (topology-cli, see below)
```

Stages 5 and 6 enforce the canary pattern: if cell-alpha's Helm install fails or the pod health check fails, the pipeline stops before deploying to any other cells. Stage 4's `helm diff` means you see exactly what's changing before anything touches production.
...
**Via the pipeline** — stage 10 is a manual `migrate-org` job. Trigger it from the GitLab UI with `ORG_ID` and `TARGET_CELL` as pipeline variables.
