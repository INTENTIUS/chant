...
`--promote-to <env>` adds one job to the pipeline that runs [`chant components promote --from <env> --to <promote-to>`](/chant/cli/components/#components-promote) once every component job has succeeded. A promote publishes from the build archive on disk, and the promote job runs on its own runner, so the archive travels with the pipeline:
...
- The `beforeScript` and `extraScript` options apply to the promote job as they do to the component jobs, and `promoteCommand` replaces its command.
...
Without `--promote-to` the generated pipeline has no promote job and no archive uploads.
