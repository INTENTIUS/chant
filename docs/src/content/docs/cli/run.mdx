...
List all discovered Ops (`*.op.ts` files) with the run state the selected runtime reports for each. With `--components`, list discovered components and their component workflows instead, which still requires `--temporal`.
