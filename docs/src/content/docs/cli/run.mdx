...
That line is the generated-file marker chant uses elsewhere, and it names the command, `--spec` included, that rewrites the file. Run the command from the repository root. GitHub files go to `.github/workflows/`, each with a `name:` that is its Op's name, so the Actions tab lists workflows by Op. Forgejo files to `.forgejo/workflows/`, and GitLab's single `ops.gitlab-ci.yml` (one job per Op) to the root, where the project's `.gitlab-ci.yml` includes it. `--output <dir>` writes somewhere else. `--format json` prints `{ files: [{ name, path, content }], jobs }` and writes nothing. An unknown Op name, a provider with no Op generator, or no scheduled Op and no `--spec` exits 1.
