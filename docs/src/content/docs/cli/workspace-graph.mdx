...
| 1 | A member failed or couldn't be read, the declaration couldn't be read, or the `--kind` records couldn't be read. The document is still printed, with the failed members listed. With `--intent`: the declaration, a kind or the region couldn't be read, or a plugin's commit join failed. |
