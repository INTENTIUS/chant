...
```json
{
  ...
  "workspace": { "name": "acme", "root": "." },
  "members": [
    { "name": "app", "dir": "app", "kind": "chant", "status": "read", "reason": null, "chant": "0.84.0", "runtimeReasons": [] },
    { "name": "delivery", "dir": "delivery", "kind": "chant", "status": "read", "reason": null, "chant": "0.84.0", "runtimeReasons": [] }
  ],
  "composites": [
    {
...
The reference workspace's `delivery` member declares the app with the docker lexicon's `DockerWebService` composite and deploys it with the `app` component, whose contract lists that kind in `composites`. Its document has one row, `delivery/app`, with that component matched `by: "composites"` and `via: "member"` and no reason. The component's only runtime is `local`.
