...
- nginx config (`NGX*`) — `nginx.conf`, and `.conf` files under nginx-ish directories (`nginx/`, `conf.d/`, `sites-available/`, `sites-enabled/`, `snippets/`) whose parsed content confirms they are nginx. Audit-only like `WRG*` (#1979): chant never writes nginx config, it only reads it for these checks. See [nginx config detection](#nginx-config-detection) below.
...
| `--max-files <n>` | Local path only: how many files the walk takes before it stops (default `1000`). See [Scan limits](#scan-limits) |
...
A local audit walks the tree once and stops after 1000 files. Every file the walk reaches counts toward that limit, including the ones it does not audit. A repository with a large `docs/` directory can therefore hit it early. Directories the walk skips (`node_modules`, `.git`, `dist`, and most dot-directories) are never counted.

Pass `--max-files <n>` to raise the limit. A repository URL is fetched with its own limits, so the flag is refused there.

When the walk stops with files left over, the report says so. Text output (`stylish`, `markdown` and `html`) leads with a note:

```
Note: The scan of ./repo stopped at 1000 files, so part of the tree was not audited. The limit counts every file the walk reaches, not only the ones it audits. Raise it with --max-files <n>.
```

JSON output carries a `truncated` field with the limit and the flag that raises it. The field is absent when the whole tree was read:

```json
{ "schemaVersion": "1.0", "status": "ok", "truncated": { "limit": 1000, "flag": "--max-files" }, "...": "..." }
```

SARIF has no place for the note, so with `--format sarif` it goes to stderr instead.

Up to `v0.80.0` a truncated scan read as a whole one on stdout, and `v0.80.0` warned about it on stderr. This is one of the level-0 changes on the [exception list](/chant/reference/level-0-exceptions/) ([#2528](https://github.com/INTENTIUS/chant/issues/2528)).

### `.gitignore` and `TF023`

`TF023` reports Terraform state and `.terraform/` directories that are committed. On a local path, a path that any `.gitignore` between it and the scan root ignores is not committed, so it is not reported. Auditing from the repository root and from a subdirectory such as `infra/` agree about `infra/terraform.tfstate` whenever the `.gitignore` that ignores it is at or below `infra/`. Up to `v0.80.0` only the scan root's own `.gitignore` was read ([#2528](https://github.com/INTENTIUS/chant/issues/2528)).
...
- An optional `truncated` object (`limit`, `flag`) means the local walk stopped at its file limit, so `findings` covers only part of the tree (see [Scan limits](#scan-limits)).
- `lineage` credits the open-source tools whose rules check the same condition, with the upstream rule id and how the two relate. It is credit, not authority, and is empty for a rule with no known counterpart. The [audit rules reference](/chant/lint-rules/audit-rules/#prior-art) lists every tool credited.
...
### Lexicons a project declares by path

A chant project can declare a lexicon by module path in `chant.config.ts` ([A lexicon in your own project](/chant/guide/managing-lexicons/#a-lexicon-in-your-own-project)). Such a lexicon has no package. The install line leaves it out, and the messages above name its path instead. Audit reads the target's config without running it, since it audits code it must not execute. When the config's `lexicons` cannot be read that way, audit prints a note saying so. Every lexicon is then treated as a package. It never loads a lexicon's module from a path either: its checks always come from installed lexicon packages. See [Commands that read `chant.config.ts` without running it](/chant/architecture/sandbox/#commands-that-read-chantconfigts-without-running-it).
...
A Terraform finding (`TF*`) is a different mechanism: it reads `# chant-ignore`, `# chant-ignore-file`, and `# chant-ignore-block` comments straight out of the `.tf` source, so it works identically through `chant audit` and `chant build`. See the terraform lexicon's [Suppressing a finding](/chant/lexicons/terraform/lint-rules/#suppressing-a-finding) section.
...
Configuration](/chant/lint-rules/configuration/#the-chant-audit-failure-severity-floor-chant-2113)
