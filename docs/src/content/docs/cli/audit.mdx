...
When the walk stops with files left over, `chant audit` prints a warning on stderr:

```
warning: the scan of ./repo stopped at 1000 files, so part of the tree was not audited. The limit counts every file the walk reaches, not only the ones it audits.
  Raise it with --max-files <n>. From the next release, text and JSON output will also report a truncated scan.
```

In this release the report is unchanged: a truncated scan still reads as a whole one on stdout, in every format. The warning goes to stderr, so `--format json` output stays byte-identical.

### Coming in the next release

Two changes to level-0 output are on the [exception list](https://github.com/INTENTIUS/chant/issues/2525) and are tracked in [#2528](https://github.com/INTENTIUS/chant/issues/2528). This release warns about both.

| Change | This release | Next release |
|---|---|---|
| Truncated scans | A warning on stderr | Text and JSON output report the truncation, the limit and `--max-files` |
| `.gitignore` for `TF023` | Only the root `.gitignore` decides whether a state file or `.terraform/` counts as committed. A warning on stderr names each path a nested `.gitignore` ignores | Every `.gitignore` between the file and the scan root is read, so those paths are no longer reported |

The `TF023` warning names the file and the `.gitignore` responsible:

```
infra/terraform.tfstate - warning: TF023 reports this path because chant audit reads only the root .gitignore, but infra/.gitignore ignores it.
...
Auditing from a subdirectory reads that directory's `.gitignore` as the root one, so today an audit from the repository root and one from `infra/` can disagree about `infra/terraform.tfstate`. After the change they agree whenever the `.gitignore` responsible is at or below the subdirectory.
