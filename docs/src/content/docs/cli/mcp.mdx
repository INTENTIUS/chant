...
Returns `{ workflowId, runId, status, startTime, closeTime, taskQueue, activitiesCompleted, activitiesScheduled, progress, gate }`. `progress` is the per-phase `StepRecord[]` array (present when the Op's `*.op.ts` is discoverable; see [Progress and gate state](/chant/guide/ops-reference/#progress-and-gate-state)) — the same shape `--progress-json` streams. `gate` is the pending gate (`{ signalName, description?, since }`) or `null` when none is blocking.
