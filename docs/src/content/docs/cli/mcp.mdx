...
Returns `{ op, runId, state, startedAt, endedAt, records, gate }`. `records` is the per-phase `StepRecord[]` array (present when the Op's `*.op.ts` is discoverable; see [Progress and gate state](/chant/guide/ops-reference/#progress-and-gate-state)) — the same shape `--progress-json` streams. `gate` is the pending gate (`{ name, since }`), absent when none is standing.
