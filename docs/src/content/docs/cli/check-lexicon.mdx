...
- **Registered intrinsics are exported** / **... `isTag` matches how they're authored** parse `src/plugin.ts`'s `intrinsics()` registrations and `src/index.ts`'s export surface (no execution — static AST analysis) to verify every registered intrinsic actually exists and that its `isTag` claim matches whether it's really authored as a tagged template.
