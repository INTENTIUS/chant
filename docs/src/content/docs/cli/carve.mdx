...
Terraform only. The advisor parses HCL with [`@cdktn/hcl2json`](https://www.npmjs.com/package/@cdktn/hcl2json), HashiCorp's HCL parser compiled to wasm and maintained by the CDK Terrain fork since the original repo was archived. It is not a chant dependency, because only carve-out users need it. The first run without it fails with an install hint:
...
A reference that reaches the carved resource through a `locals` block or a `data` source counts as a full inbound edge. Neither block is carvable, so the edge is recorded against the surviving resource that reads the local or the data source, and `carve bridge` lands the rewrite in the referrer's own body. Chained locals resolve through to whatever resource the chain ultimately names.
