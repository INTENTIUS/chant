...
chant workspace records new [<kind file>] --from <file|-> [--prefix <prefix>] [--sign [<key file>]] [--dry-run]
...
| Command | Writes |
|---|---|
| `records new <kind file> --from <file\|->` | a new record in the kind's directory, from a JSON object of its fields, sealed by its author with `--sign` |
