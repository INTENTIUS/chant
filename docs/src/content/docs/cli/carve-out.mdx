...
Adopts the selected live resource into typed chant source via the cloud→code live-import path, and reports the boundary — every dependency edge the carve would cut.

```
chant carve emit --from ./infra --select aws_s3_bucket.assets --env prod
```

It refuses to emit a resource with no native mapping (the advisor bands those "leave in Terraform"). Nothing is patched or applied; the resource lands at the observe position.
