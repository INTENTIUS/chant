...
This is the first step of the provisioned-diagram work: nodes only. Reconstructing the edges between live resources (they reference each other by physical id, not declared references) and drawing the result as an AWS-style architecture diagram are tracked separately. Scope today is **managed-only** — resources carrying chant's ownership marker; it isn't a full-account scan. Unlike the source graph, `--live` is not lint-gated (it reads the cloud, not source).
