...
`--live --overlay --traffic "<level>"` asks the project's predicting lexicon what the overlaid estate does at that traffic level, and carries the answer on the IR. Each entity's block rides on `attrs._behaviour`. The graph-level half rides on `meta._behaviour`: which engine answered, at what version and level, with the estate totals.

```bash
chant graph --format ir --live --env prod --overlay --traffic "100 rps, p50"
```
