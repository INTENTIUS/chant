...
The engine sees no credentials: the request is built from the graph the read already holds, so there is no second read for one to leak from. Predictions need `predictBehaviour` from an installed lexicon. With none the graph is served unchanged, and says so once.
