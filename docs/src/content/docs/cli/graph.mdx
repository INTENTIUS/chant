...
Scope today is **managed-only** — resources carrying chant's ownership marker; it isn't a full-account scan. Unlike the source graph, `--live` is not lint-gated (it reads the cloud, not source). Edge reconstruction is only as rich as the observed node attributes: a lexicon whose live observation returns thin metadata yields few edges until its attribute source is enriched.
