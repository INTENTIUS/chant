...
##### `--traffic "<level>"` — carry a prediction on the graph

`--live --overlay --traffic "<level>"` asks the project's predicting lexicon what the overlaid estate does at that traffic level, and carries the answer on the IR: each entity's block on `attrs._behaviour`, and the engine, its version, the level and the estate totals on `meta._behaviour`.

```bash
chant graph --format ir --live --env prod --overlay --traffic "100 rps, p50"
```

The level is passed to the engine verbatim. chant does not parse it, default it, or convert it, because every figure an engine returns is *at* a stated traffic and inventing one would silently decide what every number on the graph means. That is why the flag is the trigger rather than the engine variable:
