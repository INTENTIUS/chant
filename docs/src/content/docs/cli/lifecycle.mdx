...
Replacing and unclassified counts are also printed on **stderr**, so a `--json` or `--report gitlab-mr` consumer — whose shapes have no column for disruption — still hears about them.
