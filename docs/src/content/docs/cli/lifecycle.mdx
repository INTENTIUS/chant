...
```
aws (properties)
1 property drift across 1 resource(s), 0 accepted, 3 unchanged, 1 held elsewhere, 1 unobserved
...
| **undeclared** | `HELD ELSEWHERE` | Reported with its live value. Never drift, never counted in the drift total, never proposed for change, and never written by `--update-baseline`. |

Where the substrate records who wrote a field, the held row names them instead: Kubernetes' `metadata.managedFields` gives `spec.replicas: 7 [held by hpa-controller]`, which is the difference between a controller doing its job and somebody running `kubectl edit`. Everywhere else the declaration is the only witness and the row says the path is not in this declaration's claimed fields. `--json` carries both under `heldElsewhere`, with `source` set to `field-manager` or `claimed-fields` so a consumer can tell which answered.
...
A path inside a list is claimed by its exact position or key, so a lexicon that supplies an `orderKey` for a set-like list gets the sharper answer: a tag added in the console is `Tags[#other]`, held elsewhere, while a positional list reports the new element by index instead.
...
An `update` is bounded by the **claimed-field set**: a property the declaration never set is somebody else's field, so it is reported as [held elsewhere](#three-ways-a-live-value-classifies) by `diff --live` and never becomes an `update` here. The plan proposes closing the gap on fields chant claims and nothing more, which is why an autoscaler's replica count or a controller's annotation cannot turn into a proposal to overwrite it.
