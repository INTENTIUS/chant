...
The same engine runs as a durable step with the `envTeardown` builder — an [Op](/chant/guide/ops/) that tears an environment down on Temporal (or the local executor) instead of from a terminal:

```typescript
import { Op, phase, gate, envTeardown } from "@intentius/chant-lexicon-temporal";
...
Steps run in authored order, so a `gate(...)` placed before `envTeardown` holds the deletion until the signal arrives — the durable equivalent of pausing at the plan. The guards are the CLI's guards: an undeclared environment and a missing `ownership.stack` fail the step before any live read, and a production-like name needs `envTeardown("prod", { confirmProd: true })` — the authored counterpart of `--confirm-prod`, since an Op never prompts. The step fails when any candidate is still `failed` after the retry pass, naming the survivors; a fully clean run returns the same per-candidate report the CLI prints. See the [Ops reference](/chant/guide/ops-reference/#step-builders) for the builder row.
