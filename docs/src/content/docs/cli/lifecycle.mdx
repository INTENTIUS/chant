...
##### Three ways a live value classifies
...
| Classification | Where it appears | What chant does with it |
|---|---|---|
| **declared and equal** | counted in `unchanged` | Nothing. Source and cloud agree. |
| **declared and different** | `PROPERTY DRIFT` | Drift. The only classification that may become an `update`. |
| **undeclared** | `UNCLAIMED` | Reported with its live value. Never drift, never counted in the drift total, never proposed for change, and never written by `--update-baseline`. |

Where the substrate records who wrote a field, the unclaimed row names them instead: Kubernetes' `metadata.managedFields` gives `spec.replicas: 7 [held by hpa-controller]`, which is the difference between a controller doing its job and somebody running `kubectl edit`. Everywhere else the declaration is the only witness and the row says the path is not in this declaration's claimed fields. `--json` carries both under `unclaimed`, with `source` set to `field-manager` or `claimed-fields` so a consumer can tell which answered. It is a different array from `held`, which is the [`heldElsewhere()`](#heldelsewhere-a-field-declared-then-handed-over) set: `unclaimed` is a field source never declared, `held` is a field source declared and then handed over.
...
##### What a drifted field proposes

Drift on a declared path is the only thing that may become a change, and how it becomes one depends on where the field came from. `chant build` records that per emitted property (see [Composite Resources](/chant/guide/composite-resources/#what-a-composite-records-about-its-fields)), and the report prints a verdict under any row where editing the declared value in place would be the wrong move:
...
A property declared [`heldElsewhere()`](/chant/concepts/drift-detection/#heldelsewhere-a-field-declared-once-then-handed-over) never reports as drift, whatever the live value is, and gets its own section instead, naming who holds it and why:
...
| **update** | Declared and live, but the live config drifted — carries a [disruption verdict](#disruption-how-much-an-update-costs) saying whether applying it replaces the resource or mutates it in place |
...
An `update` is bounded by the **claimed-field set**: a property the declaration never set is somebody else's field, so it is reported as [unclaimed](#three-ways-a-live-value-classifies) by `diff --live` and never becomes an `update` here. The plan proposes closing the gap on fields chant claims and nothing more, which is why an autoscaler's replica count or a controller's annotation cannot turn into a proposal to overwrite it.
...
For every lexicon that implements `observeResourcesDeep()`, the plan also carries a `held` array, one entry per entity with at least one property declared [`heldElsewhere()`](/chant/concepts/drift-detection/#heldelsewhere-a-field-declared-once-then-handed-over). A held property is not an action, never appearing inside `entries` and never the reason an entity classifies as `update`. The human render prints its own HELD section, `--json` carries the array as `held` beside `entries`, and `--report markdown` folds it into the same section shown for `lifecycle diff --live`.
...
#### Disruption: how much an `update` costs
...
| **rolling** | The resource survives, but its workload is replaced incrementally — a Deployment's pod template changing |
| **replace** | A new resource is built and the old one removed. The physical id changes, so anything holding the old one has to be updated |
| **destroy** | Replacement that removes the old resource **first**. There is a window with nothing in it, and whatever the old one held is gone |
...
| `disruption` | How much applying an `update` costs, from the lexicon that owns the spec. Present on every `update` and on nothing else. `unknown` means nobody could say. |
| `disruptionBecause` | The attribute paths that forced the verdict — which of five changed properties is the one that replaces the resource. |
| `disruptionDetail` | The spec knowledge behind the call, or why there is none. |
...
The github lexicon ships a [`PrPlanReport`](/chant/lexicons/github/composites/#prplanreport) composite that runs the plan and posts (or updates) one sticky PR comment per environment, reusing the marker-and-`gh api` mechanism `examples/github-pr-preview` proved. The forgejo lexicon gets it for free — it inherits every github composite through its dialect re-export.
...
The [Runtime observation coverage matrix](/chant/lexicons/overview/#runtime-observation-coverage) on the Lexicons overview page is the canonical per-lexicon table, with the query mechanism and ownership channel for each. It is the one place a new lexicon adds its row.
...
**Stack names, when the project declares its stacks.** A project that sets `stacks` in `chant.config.ts` gets each declared stack's own source built and the answer keyed by stack name. Without it, one build of the source root can only answer by lexicon partition, so an aws-only estate of fifteen stacks reports "aws changed" whichever one moved, and nothing downstream can join to that. Stack names are what a component's deploy step uses, which is what makes [`chant components fan-out`](/chant/cli/components/#components-fan-out) able to consume this output. In that mode `--include-dependents` has nothing to add and prints a note saying so: the build holds cross-lexicon edges, not edges between deployed stacks, and the relation between two deployed stacks is stated by the components' `dependsOn`, which the fan-out walks for itself. An empty `dependents` there means "not computed", never "nothing downstream".
...
| Flag | Meaning |
|---|---|
| `--json` | Emit `{ environment, identities: [...] }`, one row per lexicon, with the same verdicts. |
| `--strict` | Exit 1 when any lexicon could not determine an identity. A `not reported` row never fails, since a lexicon that implements nothing is not a failure. |
...
The same engine runs as a step with the `envTeardown` builder — an [Op](/chant/guide/ops/) that tears an environment down behind a gate instead of from a terminal:
...
| `--dry-run` | Compute the rollback delta and print it as a patch on stdout. No PR is opened, nothing is pushed, no branch is left behind |
...
- [Watching Lifecycle](/chant/guide/watching-lifecycle/) — wire `lifecycle snapshot` + `lifecycle diff --live` to a cadence via the `WatchOp` composite
...
- [Implementing Observation](/chant/lexicon-authoring/observation/#describeidentity--who-chant-would-act-as) — for lexicon authors, how to answer `whoami` via `describeIdentity()`
