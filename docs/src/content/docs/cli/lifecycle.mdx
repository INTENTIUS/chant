...
chant lifecycle teardown <env> [--json]
...
Plan what deleting an environment would remove. **Plan only for now** — this command enumerates and prints; the execution half (`--yes`) is forthcoming under [#1222](https://github.com/INTENTIUS/chant/issues/1222) and currently errors.

```bash
chant lifecycle teardown dev
...
The same enumeration is exported as `planTeardown()` from `@intentius/chant` for programmatic use (a test-environment harness tearing down in `afterAll`, for example).
