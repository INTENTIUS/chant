...
A member that can't be read is still listed, with a reason code, and the command still exits 0 ([ws-020](https://github.com/INTENTIUS/chant/blob/main/docs/design/decisions/ws-020-unreadable.md)). A reader can then show the rest of the workspace and explain the gap. Failing on an unreadable member is the job of `chant workspace check`, once it checks the declaration ([#2535](https://github.com/INTENTIUS/chant/issues/2535)). Only a declaration that can't be read exits 1.
...
| `unknown-kind` | has a kind no installed kind has; the message lists the known ones |
