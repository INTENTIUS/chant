...
- **Cedar** — the source format has its own grammar, so parsing is delegated to `@cedar-policy/cedar-wasm` rather than done here; the parser's own work is turning that module's output into props. Worth reading if your format has a real parser upstream
