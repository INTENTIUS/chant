...
rather than components, and each spec can carry two things the options block
cannot, because they are per Op: `setup`, an ordered list of `{ uses, with?,
env? }` / `{ run, env? }` steps emitted ahead of the `beforeScript` lines, and
`permissions`, a map your generator merges additively over whatever the
finding mode already grants (chant #2242). A provider with no equivalent
degrades by name at build time rather than dropping the option: gitlab refuses
a `uses` entry and an additive `permissions` map, forgejo emits the steps and
drops the permissions the way it already drops the mode's own.
