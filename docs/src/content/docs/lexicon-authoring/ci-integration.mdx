...
dropping the option: gitlab refuses a `uses` entry and an additive
`permissions` map, forgejo emits the steps and drops the permissions the way
it already drops the mode's own.
