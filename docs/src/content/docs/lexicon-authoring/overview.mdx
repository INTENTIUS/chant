...
| `auditEntities(content)` | `Map<string, Declarable>` | Parse standalone manifest content into the entity graph so entity-reading checks fire on `chant audit` (parse-to-graph). Must tolerate malformed input |
