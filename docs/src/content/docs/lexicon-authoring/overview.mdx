...
| `activityContracts()` | `ActivityContract[]` | Args and return schemas for the Op activities this lexicon implements, so another lexicon's post-synth check can validate a step that calls one ([#2101](https://github.com/INTENTIUS/chant/issues/2101)). Rarely needed: the ordinary route is exporting them from `@intentius/chant-lexicon-<name>/op/activity-contracts`, which `loadActivityContracts` resolves by convention, the same way `loadActivities` resolves the implementations. This member is for a lexicon whose contracts are not reachable at that subpath |
| `activities()` | `Record<string, ActivityFn>` | The Op activity implementations, keyed by the name a step's `fn` uses ([#2520](https://github.com/INTENTIUS/chant/issues/2520)). Read only for a lexicon declared by module path in `chant.config.ts`, which has no `op/activities` subpath. A package-backed lexicon leaves it out |
...
| `predictBehaviour()` | `BehaviourResult` | Cost per hour, headroom, an error-rate expectation, a resilience verdict and a right-size hint per entity, at a stated traffic level, every figure carrying its engine, version, tolerance and `modeled`/`validated` basis. The one member of this family that reports a prediction rather than a reading, and the type keeps the two apart: money exists only as a per-hour rate for an imagined hour, the options cannot carry a credential, and a missing, unreachable, unfunded or throttled engine returns a named refusal instead of an empty report. Options mirror `observeResourcesDeep()`'s plus `traffic` and `edges`, the latter carrying `IREdge` so the declared and live paths hand the engine the same graph shape ([chant #2356](https://github.com/INTENTIUS/chant/issues/2356)) |
| `behaviourKinds` | `BehaviourKinds` | What this lexicon's own entity types are worth to a behaviour engine: `mapped` rows giving an engine kind and the declared property its size is read from, `unmapped` rows stating that a real type carries no rate and why, or `nothingPriced` saying it once for the whole substrate — a CI workflow is not an estate. Rows rather than a method, because core does the predicting and a lexicon only says what its types mean; a type this lexicon claims and has no row for is reported `unknown-type`, the one verdict that is a defect rather than a decision. Where rows are keyed on something other than the entity type — terraform's every `resource` block arrives as `Terraform::Resource` — `resolveType` names the key ([chant #2382](https://github.com/INTENTIUS/chant/issues/2382)) |
...
| `composites()` | `CompositeEntry[]` | The composites the lexicon exports, as static data: name, description, the resource kinds each bundles, and its parameters. The MCP `composites` tool, the `chant://composites` resource and `search` read it. `npm run generate:composite-catalogs` writes it to `src/composites/catalog.ts` |
| `commands()` | `CommandGroup` | A CLI verb group mounted under `chant <name> <verb>` — core dispatches to it wholesale and knows nothing about what's inside. `<name>` cannot be one of core's own command words or `workspace`, which core reserves; a plugin that tries fails to load with an error naming the word |
