...
**Seven lexicons implement this seam** — aws, azure, gcp, k8s, temporal, helm
and fountain — each shipping a `src/deep-observe.ts` plus
`deepNormalizationHooks`, both wired through its `plugin.ts`. That is the seam's
conformance evidence: seven readers over seven very different transports (the
Cloud Control API, an ARM GET, a direct GCP REST GET, a typed Kubernetes client,
a Temporal client, the helm CLI delegating to that same k8s client, a fountain
REST list) against one unchanged interface, with **no per-lexicon branch
anywhere in `lifecycle/deep-diff.ts` or `deep-observation.ts`**. An eighth is a
reader and its hooks, not a core change.
