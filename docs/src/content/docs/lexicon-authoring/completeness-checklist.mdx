...
| `Any ./workspace-kinds subpath holds valid kind data` | A lexicon that supplies [member kinds](/chant/reference/workspace-kinds/) exports them at `./workspace-kinds` as a JSON file that validates as kind data and is covered by `files` (chant #2535). Lexicons without the subpath pass vacuously |
...
Counts below are what `chant dev check-lexicon` reports at chant 0.57.0, and
