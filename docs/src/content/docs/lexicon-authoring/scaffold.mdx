...
35 files, listed here as `initLexiconCommand` writes them
...
Run `chant dev check-lexicon lexicons/acme` on a fresh scaffold and five tier-1
checks fail. Each is a real gap rather than a defect in the tool:
...
| `dist/manifest.json exists` | Run the package pipeline. The scaffolded `prepack` runs generate, validate and build, but no bundle step |
| `dist/manifest.json declares a chantVersion` | Set `chantVersion` in the manifest your `buildManifest` callback returns |
| `package.json routes exports["."].default at ./src/index.ts and deletes emitted JS in build` | The scaffold writes `"." : "./src/index.ts"` as a bare string. Expand it to a conditions object and append a `find dist -name "*.js" -delete` to the `build` script. Copy `lexicons/k3s/package.json` |
| `Every shipped example builds and passes its own post-synth checks` | The example imports `@intentius/chant-lexicon-acme`, which resolves only once the package is linked into the workspace |
