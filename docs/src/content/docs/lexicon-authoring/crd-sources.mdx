...
Every third-party API in the k8s lexicon arrived this way, from KubeRay and Argo CD to the ACK controllers. Read `CRD_SOURCES` for what is there today. This page documents how to add to it.
...
export const CRD_SOURCES: CRDSource[] = [
  ...
  // ...cert-manager, Argo, Flux, Gateway API, Prometheus, Cockroach, ACK
];
```

The `CRDSource` type is in `lexicons/k8s/src/crd/types.ts`; `loadCRDs` in
`lexicons/k8s/src/crd/loader.ts` fetches each one. Each block in
`crd-sources.ts` is preceded by a doc comment naming the operator, its API
group, the kinds it produces and the install command for its controller. Follow
...
| `{ type: "helm", chart, version, chartSubdir? }` | a chart reference | an operator that ships CRDs only in its chart |
...
Every shape also takes an optional `kinds` allowlist, which keeps a multi-document bundle down to the kinds you actually want.

Pin the operator version in a `const` next to the entry (see the `KUBERAY_VERSION` and `ARGOCD_VERSION` constants at the top of `crd-sources.ts`) so an upgrade is a one-line change and the source URL always resolves to a known revision.

### When the CRDs are only in a chart
...
> The CRD YAML is fetched and baked in at generation time, not at build time. `chant build` stays offline and deterministic — see [Network Egress](/chant/reference/network-egress/) for the guard that keeps that true.
...
| `lambda.aws.amazon.com` | `KubeMicroVM` (override) | `K8s::KubeMicroVM::MicroVM` |
| `secrets.infisical.com` | `Infisical` (override) | `K8s::Infisical::InfisicalSecret` |
| `postgresql.cnpg.io` | `Cnpg` (override) | `K8s::Cnpg::Cluster` |
| `barmancloud.cnpg.io` | `Cnpg` (override) | `K8s::Cnpg::ObjectStore` |
...
:::note
The mapping lives in `namespaceSegmentForGroup` and `GROUP_NAMESPACE_OVERRIDES` in `lexicons/k8s/src/group-namespace.ts`, not in `crd/parser.ts`. Add an override there if a group's first segment collides or reads poorly.
:::

The generator types the writable `spec` and `metadata`. `extractProperties` in `crd/parser.ts` skips `apiVersion` and `kind` (the serializer supplies both) and skips `status`, which `extractStatusType` re-attaches as a read-only `status` attribute instead of an authoring property.
...
```bash
grep -R "RayCluster" lexicons/k8s/src/generated/index.d.ts   # the type exists
