...
It is given no credential, and the contract works at that on two levels. The type declares every obvious name `?: never`, which catches the deliberate attempt at compile time. `assertNoCredentialInOptions` then walks the whole request at runtime, reading values as well as key names, which is what catches the accident. The accident is the realistic one, because `entities[*].props` is `Record<string, unknown>` straight out of the build and no type on this contract can see into it. A lexicon surfacing a connection string puts one there without deciding to.

The walk detects three things:

- a key whose name reads as a credential, at any depth, matched case- and separator-insensitively against the same pattern `redactCredentialMaterial` uses
- a value that reads as one whatever the key is called. That means a PEM block or a JWT or an `Authorization` value, and it means a token carrying the issue prefix of GitHub, GitLab, OpenAI, Stripe, Slack, AWS, Google or npm
- a password in the userinfo of anything that parses as a URL

What it does not detect is anything outside those forms. The token list is a denylist of formats somebody has added, so an issuer nobody has added still passes, and so does a bare random string. There is no entropy scoring, deliberately: this walks build output full of ids, ARNs and digests, and a heuristic that refused those would refuse real projects rather than protect them.
...
| `no-engine` | no variable in the chain names an engine |
...
`compareFigures(a, b)` classifies a pair before anything subtracts them:

| Verdict | When |
|---|---|
| `comparable` | same engine, version, tolerance, traffic level and basis. The difference is a difference in the estate |
| `mixed-basis` | same engine and level, one figure modeled and the other validated |
| `mixed-level` | same engine, different `at`. The same question asked of two different worlds |
| `mixed-engine` | the engine, its version or its stated tolerance differs. Checked first, since nothing below it matters |

Call `compareFigures` and not `compareProvenance`. The traffic level lives on the block rather than on provenance, so the provenance-only function cannot see it and will call a 100 rps figure and a 1000 rps one comparable. Within a single report the question does not arise: `behaviourReport` refuses a block whose `at` disagrees with `meta.at`, so one run means one level.
