...
| undeclared | Reported under `heldElsewhere`. Never drift. |

No lexicon implements it. Where you return `fieldOwners` the held row names the manager and says `source: "field-manager"`. Where you do not (or for a path your ownership metadata does not cover) the claim answers instead, and the row says `source: "claimed-fields"`. See [the authoring guide](/chant/lexicon-authoring/observation/#the-claimed-field-set-half-of-that-noise-has-a-general-answer).
