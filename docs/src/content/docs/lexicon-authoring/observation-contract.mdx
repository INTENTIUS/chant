...
Options mirror `observeResourcesDeep()` field for field, plus `traffic`, so a caller already driving the deep read drives this one with the same object. The engine behind it is handed the resource graph and the traffic level and nothing else. It is given no credential, and every field name for one is declared `?: never` on the options type, so passing a token is a compile error rather than a review comment.

### A prediction is never a bill

The failure this contract exists to prevent is a modeled figure being quoted as money owed. Four things make that a type error:
