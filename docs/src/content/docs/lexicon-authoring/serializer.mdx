...
1. **Identity** — `serializer.name` and `serializer.rulePrefix` are correct
2. **Empty** — `serialize(new Map())` returns empty string
...
10. **Property entities** — `kind: "property"` skipped in output
11. **DefaultLabels/Annotations entities** — skipped in output
12. **Key ordering** — canonical order (e.g., apiVersion, kind, metadata, spec)

Use `mockResource()` and `mockProperty()` helpers with `DECLARABLE_MARKER` to create test entities without constructing real resource classes. See [Testing Your Lexicon](/chant/lexicon-authoring/testing/) for the full pattern.
