...
If your lexicon needs to split resources into separate deployment units (like AWS nested stacks, Terraform modules, or Azure linked templates), use the core **child project** pattern. A child project is a subdirectory with its own barrel file (`_.ts`) that builds independently to a valid template.
...
1. **Discovery** stops at barrel boundaries — `findInfraFiles()` skips subdirectories with `_.ts`
...
With serialization in place, the next step is to [write lint rules](/lexicon-authoring/lint-rules/) for your provider.
