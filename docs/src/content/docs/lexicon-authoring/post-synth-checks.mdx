...
That registry names and links each tool once and records its licence, so add a
new tool there before crediting it. `relation` is `equivalent`, `overlaps` or
`extends`, defined in the same file. Lineage is credit rather than authority.
It never changes a rule's tier or category. A rule can have lineage without an
authority, or the reverse. The credits render on the
[audit rules reference](/chant/lint-rules/audit-rules/#prior-art) and ride along
in the JSON and SARIF reports. `scripts/prior-art-sweep.ts` reports when a
credited tool's index stops listing a cited rule.
