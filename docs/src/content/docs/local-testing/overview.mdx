...
"Offline" here means after the image is pulled: `chant emulator up` needs the container image once, and every call after that is loopback. See [Network Egress](/chant/reference/network-egress/) for what each phase reaches.
...
<LinkCard title="Test Harness — vitest" href="/chant/local-testing/testing-harness/" description="deployStack from @intentius/chant/testing: deploy per suite, assert against the live stack, destroy in afterAll." />
</CardGrid>
...
```typescript
export default { lexicons: ["aws", "azure", "gcp", "fountain"] } satisfies ChantConfig;
```
