...
## EVL006: Invalid barrel() Usage

**Fix:** Use exactly `export const $ = barrel(import.meta.dir)`.
...
**Fix:** Check for typos, then update the lexicon (`npm update`), run `npx chant update` to sync lexicon types, and `npx chant lint --fix` to regenerate barrel declarations.
