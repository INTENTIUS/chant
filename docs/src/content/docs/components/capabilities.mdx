...
| **sbom** | `generate-sbom`, `extract-config-bom` | core | artifact-type-keyed software SBOM (native SPDX/CycloneDX, hermetic lockfile backend) and IaC config-BOM (declared resources/nested stacks/external refs) into the build archive — see [Build Archive](/chant/components/build-archive/#sbom-format-agnostic-generated-at-build-time) |
