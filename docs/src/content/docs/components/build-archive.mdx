...
A config-only or infra component with no built artifact has no `generate-sbom` step to run — it skips structurally, not as a special case. A component that wants no SBOM simply omits the step.
