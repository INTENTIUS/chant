...
| Render | ✅ |  |  | Env vars | Public API reads, one name-scoped list per declared entity plus a workspace scan for orphans. Two-tier ownership: services and env groups carry `CHANT_MANAGED_BY=chant` in their env vars (Render has no tags); datastores, projects, environments, disks, domains, credentials and webhooks have no marker channel and report `unknown`, so they are never proposed for deletion |
