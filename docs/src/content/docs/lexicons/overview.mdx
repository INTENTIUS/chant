...
| Kubernetes | ✅ |  |  | Labels | `kubectl get <kind>` per declared entity; ~20 common types; cluster resolved from `k8s.profiles.<env>.context` when bound (chant #1100) — a mismatched ambient context refuses loudly rather than reading the wrong cluster |
