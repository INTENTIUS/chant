...
| Fountain | ✅ |  |  | Metadata | REST reads against the fountain API, one list per declared kind; `--owned` reads the `managed-by: chant` metadata marker, which all three kinds carry |
