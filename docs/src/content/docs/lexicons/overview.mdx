...
| Fountain | `@intentius/chant-lexicon-fountain` | 6 resources (Environment, Vault, Agent, Teammate, Schedule, Webhook) | [Reference](/chant/lexicons/fountain/) — sandboxed agent workloads and the steward that operates them; conversations are ops, not resources |
...
| Cedar | `@intentius/chant-lexicon-cedar` | Policy and Schema, plus entity and action types generated from your schema | [Reference](/chant/lexicons/cedar/) — emits `.cedar` and Cedar JSON; observes and imports against Amazon Verified Permissions |
| k3d | `@intentius/chant-lexicon-k3d` | 1 resource (Cluster) + config property types, pinned to k3d v5.9.0 | [Reference](/chant/lexicons/k3d/) — emits the `k3d.io/v1alpha5` config `k3d cluster create --config` consumes; `k3dUp`/`k3dDown` Op activities |
| k3s | `@intentius/chant-lexicon-k3s` | 3 resources (Server, Agent, Registries) | [Reference](/chant/lexicons/k3s/) — emits `config.yaml` and `registries.yaml` for `/etc/rancher/k3s/`; `k3sInstall`/`k3sUninstall` Op activities |
| Terraform | `@intentius/chant-lexicon-terraform` | one entity per HCL block, no generated resource surface | [Reference](/chant/lexicons/terraform/) — reads an existing root module's `.tf` files as-is; `terraformInit`/`terraformPlan`/`terraformApply`/`terraformShow` Op activities and the `TerraformApplyOp` composite |
| augur | `@intentius/chant-lexicon-augur` | 1 resource (Profile), no generated resource surface | [Reference](/chant/lexicons/augur/) — turns a project's resource graph into a behaviour engine's request; ships the entity-type-to-engine-kind [coverage table](/chant/lexicons/augur/coverage/) and the first `predictBehaviour()` |
...
A fourth, `predictBehaviour()`, answers a different question from the other three. Rather than reporting what a substrate holds, it reports how the declared estate would behave at a traffic level the caller names, as cost per hour and headroom per entity plus an error-rate expectation and a resilience verdict. Every figure names the engine that produced it, that engine's version and stated tolerance, and whether it was modeled from list prices or validated against a bill. The augur lexicon implements it ([chant #2357](https://github.com/INTENTIUS/chant/issues/2357)) and is the only lexicon that does: it reads whatever the other lexicons declared, so a project adds it beside them rather than instead of one. See [the behaviour contract](/chant/lexicon-authoring/observation-contract/#predictbehaviour).

| Lexicon | `describeResources()` | `observeResourcesDeep()` | `listArtifacts()` | `predictBehaviour()` | Ownership | Notes |
|---|---|---|---|---|---|---|
...
| Terraform | ✅ |  |  |  | State file, or the marker tags on a choudoufu root | `terraform show -json` over each configured root's current state, matched against the `<root>/<address>` keys `buildRoots()` produced (chant #2087). On a stock root ownership is state membership, not a marker: that root is on the trusted-state-file row of [lifecycle models](/chant/concepts/lifecycle-models/)'s third axis, so an address the state carries is `owned` and everything else is `unknown` (a declared `module` block, which state has resources under but no row for, lands there). Only `resource` and `data` blocks have rows at all; a `terraform`/`provider`/`variable`/`output` block reports `unsupported-kind`. A root under `binary: "choudoufu"` that declares an estate is read by `choudoufu live-plan -json` instead and sits on the live-marker row like every other lexicon (chant #2104): its `tofu-estate`/`tofu-address` tags are the answer, a bound instance is `owned`, an unowned resource at a declared identity is `foreign`, an adoptable match is `unknown` carrying the two tag values that would claim it, and `live-ls -estate -json` enumerates the owned orphans for `observeAmbient`/`teardownOwned`. A root whose read fails reports every entity it declares `read-failed`, or `no-credentials` where the failure names credentials, naming the root |
| augur |  |  |  | ✅ | — | The behaviour engine's request, built offline from whatever the other lexicons declared. No substrate to read, so none of the three reads is declared: augur holds nothing and asks an engine what would happen at a stated traffic level. Its [coverage table](/chant/lexicons/augur/coverage/) says which entity types reach the engine and which are declared unmapped, and a declared-unmapped kind is reported `unsupported-kind` naming the kind rather than priced at zero. The engine address comes from `CHANT_BEHAVIOUR_ENGINE_AUGUR` / `CHANT_BEHAVIOUR_ENGINE` / `BEHAVIOUR_ENGINE`; a command on PATH is dialled with an environment holding `PATH` alone, and a URL is dialled by core's HTTP transport with a bearer token from `CHANT_BEHAVIOUR_TOKEN_AUGUR` / `CHANT_BEHAVIOUR_TOKEN` / `BEHAVIOUR_TOKEN` (chant #2359) |
