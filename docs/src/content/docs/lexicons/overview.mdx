...
| OpenTelemetry Collector | `@intentius/chant-lexicon-otel` | 18 components (4 receivers, 6 processors, 5 exporters, 3 extensions), Pipeline, Service | [Reference](/chant/lexicons/otel/) — emits collector YAML; `defineComponent` adds components chant doesn't ship, with a pinned schema |
| systemone | `@intentius/chant-lexicon-systemone` | no resources: verbs only | [Reference](/chant/lexicons/systemone/) — the `decide` Op activity asks a workspace's decision points (ws-058) through a Jev-compatible `POST /v1/systemone` backend and records the answer |
...
| OpenTelemetry Collector |  |  |  |  | — | N/A — a collector config file is an input to a collector the platform lexicons deploy; there is nothing live to read |
