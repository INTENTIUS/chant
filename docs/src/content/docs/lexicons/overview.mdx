...
| Terraform | ✅ |  |  | State file | `terraform show -json` over each configured root's current state, matched against the `<root>/<address>` keys `buildRoots()` produced (chant #2087). Ownership is state membership, not a marker: terraform is the one lexicon on the trusted-state-file row of [lifecycle models](/chant/concepts/lifecycle-models/)'s third axis, so an address the state carries is `owned` and everything else is `unknown` (a declared `module` block, which state has resources under but no row for, lands there). Only `resource` and `data` blocks have rows at all; a `terraform`/`provider`/`variable`/`output` block reports `unsupported-kind`. A root whose `init` or `show` fails reports every entity it declares `read-failed`, naming the root |
