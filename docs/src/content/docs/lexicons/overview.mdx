...
|---|---|---|---|---|---|
| AWS | ✅ | ✅ |  | Tags | `aws cloudformation describe-stack-resources`; deep read via the Cloud Control API for S3 buckets, IAM roles/managed policies and security groups. `--owned` available on the deep read (Cloud Control returns tags) and on live export, not on describe |
...
| GCP (Config Connector) | ✅ |  |  | Labels | `kubectl get <gvk>` against the GKE management cluster; cluster resolved from `k8s.profiles.<env>.context` when bound (chant #1100), else ambient — visibly |
