...
| `from.template` | no | The template id the migration applies to, as the lock writes it. Omitted, it applies to the template it ships in, whatever id a project recorded for it. Naming another template makes it a [bridge](#bridge-migrations). |
...
A bridge brings a scope made from another template onto this one. Its `from.template` names that other template, such as a fork, and its `to` is a version of this template. It runs only when [`chant workspace upgrade --source`](/chant/cli/workspace-upgrade/#moving-to-another-template) moves such a scope here.
...
```json
{
  ...
  "from": { "template": "github.com/someone/starter-fork", "versions": ">=1.0.0 <2.0.0" },
  "to": "2.0.0",
  "body": {
    "type": "declarative",
    "steps": [
