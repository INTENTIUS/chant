...
|---|---|
| [`chant init --from <repo>@<ref>[#<member>]`](/chant/cli/init/#from-a-template-repository) | Writes a new lock with scope `"."`, its `parameters` from `--param` and the template's defaults, and `repinned` when it re-pinned a record |
