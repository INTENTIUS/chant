...
The lineage lock records where a project's files came from. `chant init --from`, `chant init --template` and `chant vendor migrate` write it, and `chant vendor pull` keeps it current. Later, `chant workspace upgrade` ([#2550](https://github.com/INTENTIUS/chant/issues/2550)) reads it to bring a project forward to a newer template. The design is D9 of [#2524](https://github.com/INTENTIUS/chant/issues/2524), with decisions ws-038 (vendor as a lineage scope) and ws-047 (the lock is written at init).
...
| `ref` | The pin as written: a git ref, a tag or a version label. |
...
| `migrations` | The migrations applied since the scope was instantiated. Empty until `chant workspace upgrade` exists. |
...
|---|---|---|
| `owned` | Replaced when unedited. When edited, kept, and a manual step if the source changed it too. | every file not listed below |
| `generated` | Rebuilt by `command`, never merged. Edits and absence are not reported. | `skills/*/SKILL.md`, rebuilt by `chant update` |
...
When an update finds that the project and the source both changed a file, it keeps the project's file and records a manual step. The source's version is not written anywhere. Fetch it from the source to merge by hand.
...
Open manual steps fail `chant vendor check` under `CI`. `chant workspace lineage` lists them.

## Update rules

`chant vendor pull` applies these rules per path today. `chant workspace upgrade` will apply the same rules and add the three-way merge from ws-005.
