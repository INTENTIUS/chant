...
A reader such as hud reads only through this contract and writes only through chant commands ([ws-052](https://github.com/INTENTIUS/chant/blob/main/docs/design/decisions/ws-052-chant-hud-boundary.md), the [chant and hud boundary](/chant/reference/boundary/)). `@intentius/chant-test-utils` exports `describeWorkspaceReaderConformance`, beside `describeWorkspaceKindConformance`, so a reader can show that in its own CI.

The suite builds the reader from a function you pass, giving it a transport that runs chant in the workspace. The reader's `read(command, args)` runs one contract command through the transport and returns the parsed document.

```ts
import { describeWorkspaceReaderConformance, type ChantTransport, type ReadContractCommand } from "@intentius/chant-test-utils";
...
It reads the reference workspace with each of `ls`, `graph`, `check`, `status`, `records` and `graph --intent`, and fails when any of these is false:

| Check | How |
|---|---|
| the document validates against the command's output schema and carries this contract's version | the schemas under `packages/core/src/workspace/` |
...
The reader never gets the workspace's path, so the transport is its only route to the workspace, and the transport records everything that goes through it. `workspaceDir` names another copy of the reference workspace, and `chantCommand` names the chant to run, which is this checkout's CLI by default.
