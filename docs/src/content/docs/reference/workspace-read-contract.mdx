...
| `seal-missing` | seal, `records` | The verdict carries no seal. |
| `seal-signer-unlisted` | seal, `records` | The reviewer has no key in the signers file at base, so the seal can't count. |
| `seal-signature-invalid` | seal, `records` | The seal is malformed, names a signer other than the reviewer, or its signature doesn't verify over the verdict. |
...
A write never makes the caller compute anything about the record. `new` allocates the id, `amend` applies the approval rule, and `review` computes the digest the verdict is bound to. The principal a review names is recorded as the caller gives it. `review --sign` seals the verdict with an ssh key, and `records` then reports whether the seal verifies for that principal against the signers at base ([#2687](https://github.com/INTENTIUS/chant/issues/2687)). The provenance of the commit that carries the file is reported apart from that ([#2547](https://github.com/INTENTIUS/chant/issues/2547)).
