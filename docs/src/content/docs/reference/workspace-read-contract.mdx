...
| `work-item-closed` | error, `work claim` | A claim on a work item in a closed state, such as done or dropped: there is no work left to claim. |
| `lease-held` | refusal, `work` | Someone holds a live lease on the work item: another worker, or, for a claim, the same one. |
| `lease-not-held` | refusal, `work` | Nobody holds a live lease on the work item: it expired, was released or was never claimed. |
| `lease-token-mismatch` | refusal, `work` | The live lease on the work item carries another fencing token than the one given. |
