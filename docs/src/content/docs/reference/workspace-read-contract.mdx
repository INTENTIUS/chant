...
|---|---|---|---|
| 1 | 0.81.0 | schema 1 | `ls`, `graph`, `check`, `status`, `records`, `records-since`, `intent`, `composites` and `points`, each at `v1` |
...
| `work-item-unknown` | error, `work` | No work record has the id, so there is nothing to lease. |
