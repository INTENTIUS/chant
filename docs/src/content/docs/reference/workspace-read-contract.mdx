...
| `commit` | `commit:<sha>` | `sha`, `subject`, `author`, `date`, `trailers`, `pullRequest`, `signature` (the provenance level, judged by the signers at base as `records` judges a record) and `lines`, the ranges it changed in a line-range region |
...
| `finding` | `finding:<code>:<n>` | `code`, `message`, and `concerns`, the ids of the nodes it is about |
...
A decision covers a commit from the commit that added its record until the commit that added the record superseding it. A commit outside every path-granularity window is `intent-commit-undecided`. A decision that constrains a path below a directory region has a `constrains` edge to each file under that path, and it does not count as constraining the whole region.
