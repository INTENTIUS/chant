...
| `work-done-gap-open` | warning, `graph --intent` | A work record is done, and the finding its `source` names still fires on its region. Only `graph --intent` walks a region, so only it raises this. |
