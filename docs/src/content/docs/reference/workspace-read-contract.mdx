...
| `decision` | `record:<kind>/<id>` | `recordKind`, `record`, `path`, `title`, `state`, `closed`, `valid`, `reasons`, `provenance`, `decided_by`, `decided_on`, `reviews` (agree, dissent, abstain and open concerns), `supersededBy`, `supersedes`, and `constrains`, the entries that cover the region with their granularity |
