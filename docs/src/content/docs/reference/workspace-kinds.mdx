...
Each kind also says which outputs its members expose to [member links](/chant/reference/workspace-declaration/#member-links). A `chant` member exposes the outputs its source declares, a nested `workspace` exposes none, and an `other` member exposes what its entry lists.
...
| `kinds[].outputs` | Optional. Output names every member of the kind exposes to [member links](/chant/reference/workspace-declaration/#member-links). A member entry of the kind can list more in its own `outputs` |
