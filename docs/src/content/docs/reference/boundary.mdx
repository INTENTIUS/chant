...
| the read contract: versioned JSON of `ls`, `graph`, `check`, `records`, `status`, `graph --intent`, `graph --composites`, closed reason codes | identity, sessions, who is looking | agent prompts and context bundles |
...
| workspace command | `commandRegistry` in `cli/main.ts`, and each sub-verb its help names | 22 | 0 | 0 |
| member kind | `BUILTIN_KIND_NAMES` in `kinds.ts` | 4 | 0 | 0 |
| record kind | none in core; `decision` and `session` are this repository's, the rest a plugin's | 3 | 0 | 4 |
...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 109 | 0 | 0 |
| finding code | the finding codes of `intent.schema.json` | 17 | 0 | 0 |
| WSP check | `WORKSPACE_CHECKS` in `checks.ts` and `checks/*.ts` | 39 | 0 | 0 |
...
| boundary cell | the read contract: versioned JSON of `ls`, `graph`, `check`, `records`, `status`, `graph --intent`, `graph --composites`, closed reason codes | chant | ls.schema.json, graph.schema.json, check.schema.json, records.schema.json, status.schema.json, intent.schema.json, composites.schema.json | The only way a reader sees a workspace: one JSON Schema per document, a contract version and a chant floor. |
...
| reason code | `not-a-git-repository` | chant | check.schema.json, composites.schema.json, graph.schema.json, intent.schema.json, ls.schema.json, records-since.schema.json, records.schema.json, status.schema.json, work-lease.schema.json | --at, or a ledger read, needs a git repository and there is none. |
| reason code | `revision-unknown` | chant | check.schema.json, composites.schema.json, graph.schema.json, intent.schema.json, ls.schema.json, records-since.schema.json, records.schema.json | --at names no commit. |
...
| reason code | `record-unparseable` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | No front matter, a YAML error or a value outside the JSON subset of YAML, or for a JSON kind a file that is not one object or repeats a member name. |
| reason code | `record-schema-invalid` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | The record's front matter, or its JSON object, does not match the kind's schema. |
| reason code | `record-id-duplicate` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | Another record earlier in path order has the same id. |
| reason code | `record-supersedes-unknown` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A supersedes link names an id no record has. |
| reason code | `record-supersedes-conflict` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A second closed record supersedes a record another one already superseded. |
| reason code | `session-seal-mismatch` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A closed session's seal is not the digest of its text: the session changed after it closed. |
| reason code | `session-verdict-unknown-record` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A session's verdict names a record that none of the session kind's subject records has. |
| reason code | `asset-drift` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins by hash has changed: its bytes no longer hash to the pinned sha256. |
| reason code | `asset-missing` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins by hash does not exist in the tree read. |
| reason code | `asset-stale` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins is unchanged at the hash a record it supersedes pinned: the decision changed and the artifact did not follow. |
| reason code | `record-no-evidence` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | The record's evidence list is empty: it cites nothing and pins no file. Information for a reviewer, never an error. |
| reason code | `review-undigested` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A verdict names no digest of the text it judged. It still counts, and an amendment does not stop it counting. |
| reason code | `source-transcript-drift` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | The record's source block pins a transcript by hash, the file it names can be read here, and its bytes hash to something else: it is not the transcript the record means. |
...
| reason code | `record-unattested` | chant | records.schema.json | A signers file is active at base, and the record names an author whose seal does not verify: it has none, the author has no key in the file, or the signature fails. |
...
| reason code | `record-supersedes-pending` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A supersedes link from a record whose state is weaker than the record it names, so the link has no effect yet. |
| reason code | `kind-unreadable` | chant | intent.schema.json, ls.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json, work-lease.schema.json | The record kind file is missing or could not be imported. |
| reason code | `kind-invalid` | chant | intent.schema.json, ls.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record kind file exports no recordKind, or its shape is wrong. |
| reason code | `schema-unreadable` | chant | intent.schema.json, ls.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The schema file the record kind names is missing or is not JSON. |
| reason code | `schema-id-mismatch` | chant | intent.schema.json, ls.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The schema's $id differs from the id the record kind names. |
| reason code | `schema-invalid` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record schema itself does not compile. |
| reason code | `location-missing` | chant | intent.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The records directory does not exist, in the tree or at the revision. |
| reason code | `write-usage-invalid` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, work-lease.schema.json | The command line lacks a value the write needs, or gives one it does not take. |
| reason code | `write-input-invalid` | chant | records-amend.schema.json, records-new.schema.json | The fields given with --from or --set can't be read, are not JSON, or are not a JSON object. |
| reason code | `record-not-found` | chant | records-amend.schema.json, records-close.schema.json, records-review.schema.json | No record of the kind has the id given. |
| reason code | `record-id-taken` | chant | records-new.schema.json | The id given for a new record is already used, by a record or a file name. |
...
| reason code | `record-closed` | chant | records-amend.schema.json, records-close.schema.json, records-review.schema.json | The record is in a closed state, so nothing in it changes; a new record supersedes it instead. |
| reason code | `amend-id-immutable` | chant | records-amend.schema.json | An amendment changes the record's id, and ids are never renumbered. |
| reason code | `amend-supersede-instead` | chant | records-amend.schema.json | The record is approved, and the amendment changes a field the approval rule does not let change in place; a new record supersedes it instead. |
...
| reason code | `record-sign-failed` | chant | records-amend.schema.json, records-new.schema.json | --sign was given and no author seal could be made: the record names no author, the key can't be read or used, git names no ssh signing key, or ssh-keygen is not installed. |
...
| reason code | `work-kind-missing` | chant | work-lease.schema.json | No work kind to find the item in: --kind names a kind with no work block, or the declaration names no work kind. |
| reason code | `work-kind-ambiguous` | chant | work-lease.schema.json | More than one declared work kind has a record with the id, so --kind must name one. |
| reason code | `work-item-unknown` | chant | work-lease.schema.json | No work record has the id, so there is nothing to lease. |
| reason code | `work-item-closed` | chant | work-lease.schema.json | A claim on a work item in a closed state, such as done or dropped: there is no work left to claim. |
| reason code | `lease-held` | chant | work-lease.schema.json | Someone holds a live lease on the work item: another worker, or, for a claim, the same one. |
| reason code | `lease-not-held` | chant | work-lease.schema.json | Nobody holds a live lease on the work item: it expired, was released or was never claimed. |
| reason code | `lease-token-mismatch` | chant | work-lease.schema.json | The live lease on the work item carries another fencing token than the one given. |
