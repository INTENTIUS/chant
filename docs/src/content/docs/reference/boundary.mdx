...
| workspace command | `commandRegistry` in `cli/main.ts`, and each sub-verb its help names | 21 | 0 | 0 |
...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 95 | 0 | 0 |
...
| reason code | `kind-unreadable` | chant | intent.schema.json, ls.schema.json, records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record kind file is missing or could not be imported. |
...
| reason code | `write-usage-invalid` | chant | records-amend.schema.json, records-close.schema.json, records-new.schema.json, records-review.schema.json | The command line lacks a value the write needs, or gives one it does not take. |
...
| reason code | `box-credential-declared` | chant | check.schema.json | A file in a box member's directory carries a literal secret: a credential's shape, or a literal where a credential goes. A ${VAR}, $VAR or secret-manager reference is not one. |
...
| WSP check | `WSP121` `box-credential-declared` | chant | check.schema.json | A box holds no credential: no file in a box member's directory carries a literal secret, in its declarations, an env or a vault's defaults. ${VAR}, $VAR and secret-manager references (op://, bws://, infisical://) are not secrets. |
