...
| workspace command | `commandRegistry` in `cli/main.ts`, and each sub-verb its help names | 20 | 0 | 0 |
| member kind | `BUILTIN_KIND_NAMES` in `kinds.ts` | 4 | 0 | 0 |
| record kind | none in core; `decision` and `session` are this repository's, the rest a plugin's | 2 | 0 | 4 |
...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 69 | 0 | 0 |
...
| boundary cell | the writing and verifying commands: `approve`, `records pin`, `verify`, `check`, gates, ledgers | chant | chant approve, chant workspace records pin, chant workspace records new, amend and review, chant workspace verify, chant workspace check | Every change to the repository that carries weight is made or checked by a chant command, so a UI writes only by calling one. |
...
| workspace command | `workspace records` | chant | records.schema.json, records-since.schema.json | Reads the records a record kind locates, validated against its schema, with provenance and reason codes; with --since, what changed in them between two revisions. |
...
| workspace command | `workspace records new` | chant | records-new.schema.json | Writes one new record in a kind's directory from validated JSON fields, allocating the next id, and never commits. |
| workspace command | `workspace records amend` | chant | records-amend.schema.json | Sets fields of one record under its schema and the approval rule, refusing a change to an approved record that only a superseding record can make. |
| workspace command | `workspace records review` | chant | records-review.schema.json | Appends a dated review verdict to one record, bound to the digest of the record text, with a note required for a dissent. |
...
| reason code | `not-a-git-repository` | chant | check.schema.json, composites.schema.json, graph.schema.json, intent.schema.json, ls.schema.json, records-since.schema.json, records.schema.json, status.schema.json | --at, or a ledger read, needs a git repository and there is none. |
...
| reason code | `record-unparseable` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | No front matter, a YAML error, or a value outside the JSON subset of YAML. |
| reason code | `record-schema-invalid` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | The front matter does not match the kind's schema. |
| reason code | `record-id-duplicate` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | Another record earlier in path order has the same id. |
| reason code | `record-supersedes-unknown` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A supersedes link names an id no record has. |
| reason code | `record-supersedes-conflict` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A second closed record supersedes a record another one already superseded. |
| reason code | `session-seal-mismatch` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A closed session's seal is not the digest of its text: the session changed after it closed. |
| reason code | `session-verdict-unknown-record` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A session's verdict names a record that none of the session kind's subject records has. |
| reason code | `asset-drift` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins by hash has changed: its bytes no longer hash to the pinned sha256. |
| reason code | `asset-missing` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins by hash does not exist in the tree read. |
| reason code | `asset-stale` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A file the record pins is unchanged at the hash a record it supersedes pinned: the decision changed and the artifact did not follow. |
| reason code | `record-no-evidence` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | The record's evidence list is empty: it cites nothing and pins no file. Information for a reviewer, never an error. |
| reason code | `review-undigested` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A verdict names no digest of the text it judged. It still counts, and an amendment does not stop it counting. |
...
| reason code | `review-unattested` | chant | records.schema.json | An attestation policy is active at base, and the verdict carries no seal. |
...
| reason code | `runtimes-config-unreadable` | chant | composites.schema.json | The member's chant.config.ts could not be read, so only the built-in local runtime is listed. |
...
| reason code | `record-supersedes-pending` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json, records.schema.json | A supersedes link from a record whose state is weaker than the record it names, so the link has no effect yet. |
| reason code | `kind-unreadable` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record kind file is missing or could not be imported. |
| reason code | `kind-invalid` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record kind file exports no recordKind, or its shape is wrong. |
| reason code | `schema-unreadable` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The schema file the record kind names is missing or is not JSON. |
| reason code | `schema-id-mismatch` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The schema's $id differs from the id the record kind names. |
| reason code | `schema-invalid` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The record schema itself does not compile. |
| reason code | `location-missing` | chant | intent.schema.json, records-amend.schema.json, records-new.schema.json, records-review.schema.json, records-since.schema.json, records.schema.json | The records directory does not exist, in the tree or at the revision. |
| reason code | `write-usage-invalid` | chant | records-amend.schema.json, records-new.schema.json, records-review.schema.json | The command line lacks a value the write needs, or gives one it does not take. |
...
| reason code | `record-not-found` | chant | records-amend.schema.json, records-review.schema.json | No record of the kind has the id given. |
...
| reason code | `record-closed` | chant | records-amend.schema.json, records-review.schema.json | The record is in a closed state, so nothing in it changes; a new record supersedes it instead. |
...
| reason code | `since-rev-unknown` | chant | records-since.schema.json | --since names no commit. |
