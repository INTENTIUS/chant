...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 76 | 0 | 0 |
| finding code | the finding codes of `intent.schema.json` | 15 | 0 | 0 |
...
| reason code | `work-done-gap-open` | chant | intent.schema.json | A work record is done, and the finding it came from still fires on its region. Only graph --intent raises it. |
