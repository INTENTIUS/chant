...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 122 | 0 | 0 |
...
| reason code | `work-item-unknown` | chant | changes.schema.json, work-lease.schema.json | No work record has the id, so there is nothing to lease. |
