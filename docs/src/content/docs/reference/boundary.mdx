...
| reason code | `REASONS` in `reason-codes.ts`, less the finding codes | 133 | 0 | 0 |
| finding code | the finding codes of `intent.schema.json` | 17 | 0 | 0 |
| WSP check | `WORKSPACE_CHECKS` in `checks.ts` and `checks/*.ts` | 47 | 0 | 0 |
...
| reason code | `box-intent-unconstrained` | chant | check.schema.json | The decision record a box names as its intent constrains nothing of the box: no member: entry for it and no path: entry at, above or inside its directory. |
...
| WSP check | `WSP127` `box-intent-unconstrained` | chant | check.schema.json | The decision record a box names as its intent constrains the box, with member: and its name or a path: entry at, above or inside its directory, so graph --intent shows it for the box's files. |
