...
The triage table leaves a finding a work item already addresses. It makes `intent-decision-unimplemented` a work item, and it sends `intent-region-unconstrained`, `intent-decision-contested` and `intent-decision-superseded-live` to a decision. Any other finding goes to the model, then to people. The `needs-a-decision` table says no when nothing changed in the window, or when every commit in it is a decision's own work.
