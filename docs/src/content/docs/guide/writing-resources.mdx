...
chant reads TypeScript as data. By default, `chant build` skips execution entirely for files that stay inside the supported subset, reducing them straight from the AST (see [Folded vs Run](/chant/concepts/typescript-as-data/#folded-vs-run)). A file outside that subset is imported and run instead, and the evaluability rules (below) are what keep that run side-effect-free, so it behaves like reading static data. Either way, resource properties must be **statically evaluable** — the build must be able to resolve every value without depending on anything outside the source.
...
For the full list of supported and unsupported patterns, see [TypeScript as Data](/chant/concepts/typescript-as-data/).
