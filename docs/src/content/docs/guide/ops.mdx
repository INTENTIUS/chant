...
Whatever the target, the apply reports one result shape: the count projection of core's apply contract ([#1446](https://github.com/INTENTIUS/chant/issues/1446)) — `applied`, `pruned`, and `notAttempted` (declared resources no provider call was made for, including kinds an owned-only prune could not consider). A workflow gates on those counts regardless of target; the target-specific detail — the field manager on `kubectl`, the stack status on `cloudformation` — stays on the activity log.
...
### Scheduling without Temporal

`schedule` above produces a `TemporalSchedule`, so it needs a Temporal server. A project that doesn't run one can still put an audit/reconcile Op on a cron via CI itself — the Op counterpart to [generate mode](/chant/components/orchestration/#generate-mode)'s component pipeline. `generateOpsPipeline` (`@intentius/chant/op`) validates that every named Op exists, then delegates to the target lexicon's `generateOpPipeline` (gitlab, github, forgejo), which synthesizes one cron-triggered CI file per Op — `chant run <name>` on the local executor, with only the token/permission scope its finding-mode needs (read-only for `report`; scoped write for `issue`/`pull-request`/`merge-request`).
