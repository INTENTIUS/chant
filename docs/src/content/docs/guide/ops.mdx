...
A gate stops the run until someone records a resolution on the gate ledger. Typical gate scenarios:
...
The second run reads the resolution, walks through the gate carrying the approver onto the step record, and finishes. `chant run approve <op> <gate> --on <lexicon>` does the same write and then tells the hosting runtime, so a run parked on a steward's thread picks it up now rather than on its next tick. See [Gate-as-fact](/chant/guide/operator/#gate-as-fact) for the ordering rules.
