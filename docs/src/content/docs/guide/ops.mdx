...
`target` picks the mechanism, and there are three: `kubectl` is a server-side apply through the k8s lexicon's typed client, while `cloudformation` and `arm` shell out to `aws cloudformation deploy` and `az deployment group create`.

chant also ships four **native per-resource appliers** that speak each provider's own API directly, with no CLI in the path and an endpoint override so the same step targets a real account or an emulator: `awsApply`, `azApply` and `gcpApply` (typed step builders, like the ones in [Phases and steps](#phases-and-steps)), and `flyApply` (an activity the fly lexicon contributes, reached by name). None of them is reachable through `ApplyOp` — `target` has no value for GCP or Fly, and the two shell targets do not route to their applier ([#1449](https://github.com/INTENTIUS/chant/issues/1449)). Until that changes, use them as steps in a hand-written Op.
