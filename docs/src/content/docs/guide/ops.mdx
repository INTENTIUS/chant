...
| `arm` | `az deployment group create --mode Complete` | the resource group, **not** the ownership marker |

The first two are owned-only by construction. The ARM target is not, and the difference is destructive.

<Aside type="caution" title="`arm` deletes at resource-group scope">
ARM Complete mode removes every resource in the resource group that the template does not declare — including resources chant never applied. `delete: "owned-only"` and `delete: "gated"` both select it, so neither is owned-only on this target.

Until [#1448](https://github.com/INTENTIUS/chant/issues/1448) lands, either keep `delete: "never"` on `arm` targets, or give the stack a resource group it is the sole occupant of. The azure lexicon's own `azApply` activity prunes marker-scoped and can be called directly from a hand-written Op in the meantime.
</Aside>
