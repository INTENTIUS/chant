...
`generateOpsPipeline` (`@intentius/chant/op`) validates that every named Op exists, then delegates to the target lexicon's `generateOpPipeline` (gitlab, github, forgejo), which synthesizes one cron-triggered CI file per Op — `chant run <name>` in the job, with only the token/permission scope its finding-mode needs (read-only for `report`; scoped write for `issue`/`comment`/`pull-request`/`merge-request`). This is the Op counterpart to [generate mode](/chant/components/orchestration/#generate-mode)'s component pipeline.
...
Any non-`report` mode on a `pull_request` trigger also gets `pull-requests: write`, which is why `comment` costs nothing beyond what that trigger already grants. `comment` is the one mode that constrains the trigger as well as the scope: the github generator refuses it by name on a cron or push trigger, and the gitlab and forgejo generators refuse it outright, since the activity behind it shells to `gh` against the GitHub API and chant carries no client for the other two forges.
