...
`target` picks the mechanism, and every one of them is native — no cloud CLI in the path: `kubectl` and `kustomize` are a server-side apply through the k8s lexicon's typed client, `arm` is the azure lexicon's per-resource `azApply`, `cloudformation` is the aws lexicon's `awsApply`, which speaks the CloudFormation API directly, and `gcp` is the gcp lexicon's per-resource `gcpApply`, which maps each CNRM kind to its GCP REST API. Each is loaded from its lexicon at apply time, so a project using a target lists that lexicon in `chant.config.ts`.

chant also ships four **native per-resource appliers** that speak each provider's own API directly, with no CLI in the path and an endpoint override so the same step targets a real account or an emulator: `awsApply`, `azApply` and `gcpApply` (typed step builders, like the ones in [Ops Reference](/chant/guide/ops-reference/#step-builders)), and `flyApply` (an activity the fly lexicon contributes, reached by name). `awsApply`, `azApply` and `gcpApply` are exactly what `ApplyOp` runs for the `cloudformation`, `arm` and `gcp` targets. Fly has no `target` value yet ([#1449](https://github.com/INTENTIUS/chant/issues/1449)); until it does, use `flyApply` as a step in a hand-written Op.
