...
- **No ledger branch.** A CI checkout fetches the pipeline's own ref and nothing else, so `chant/lifecycle` is absent, and an absent branch reads exactly like an empty ledger: like nothing having been approved. A retried job would then record a *second* pending fact for a gate somebody had already answered. `chant run` and `chant approve` fetch the branch before reading it, and refuse by name if they cannot, rather than acting on history they could not see. `GIT_DEPTH: 0` does not substitute for this on GitLab: it fetches refspecs, not all branches.
