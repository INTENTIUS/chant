...
The `ApplyOp` composite is the other direction: compute the plan, then apply via the target's native mechanism — a Kubernetes server-side apply, CloudFormation `deploy`, or an ARM deployment. Authority stays with the platform; chant hosts no state file.
...
`delete` controls how apply treats resources no longer declared. Deletes ride the target's **native** delete path scoped to the ownership marker — for Kubernetes, a sweep filtered by `app.kubernetes.io/managed-by=chant` (and by the stack when `ownership.stack` is set), restricted to the namespaces the apply touched; for CloudFormation, the stack boundary; for ARM, `--mode Complete`. Because every delete path is marker-scoped, a foreign (unmarked) resource is never touched: deletes are owned-only by construction.
