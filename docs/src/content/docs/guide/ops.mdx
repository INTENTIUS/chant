...
- **No committer identity.** GitLab CI, GitHub Actions and Forgejo Actions all clone without `user.email` or `user.name`, and `git commit-tree` refuses rather than inventing one. chant supplies its own — `chant <chant@localhost>` — for that commit alone, applied per-invocation and only when the checkout cannot name a committer itself. A configured identity is never overridden, and nothing is written into the repository's config. Set `CHANT_LIFECYCLE_COMMITTER_NAME` and `CHANT_LIFECYCLE_COMMITTER_EMAIL` to attribute these commits to a bot account you control instead.
- **No ledger branch.** A CI checkout fetches the pipeline's own ref and nothing else, so `chant/lifecycle` is absent — and an absent branch reads exactly like an empty ledger, which is to say like nothing having been approved. A retried job would then record a *second* pending fact for a gate somebody had already answered. `chant run` and `chant approve` fetch the branch before reading it, and refuse by name if they cannot, rather than acting on history they could not see. `GIT_DEPTH: 0` does not substitute for this on GitLab: it fetches refspecs, not all branches.

A job that cannot reach the remote at all still gates correctly the first time; what it will not do is treat an unreadable ledger as an empty one.
