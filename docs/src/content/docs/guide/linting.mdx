...
1. **Evaluability rules (EVL)** — ensures all resource property expressions can be statically evaluated. See [Evaluability Rules](/lint-rules/evaluability/).
2. **Correctness & style rules (COR)** — structural constraints like missing exports, circular dependencies, naming conventions. See [Correctness Rules](/lint-rules/correctness/).
...
Rules are configured in `chant.config.ts`. See [Rule Configuration](/lint-rules/configuration/).
