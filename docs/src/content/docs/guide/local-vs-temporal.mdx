...
**One shape of Op is the exception to "crash survival needs Temporal": `ConvergeOp`.**
A converge tick is level-triggered — it re-observes and re-derives everything
every time, so durability costs nothing extra: crash recovery and a normal
tick are the same act. [`chant operator`](/chant/guide/operator/) runs
scheduled converge ticks locally, durably, with no Temporal server — see that
guide for the full pipeline-vs-converge distinction and when each still
belongs to Temporal.
