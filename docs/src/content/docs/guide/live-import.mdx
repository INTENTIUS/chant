...
1. Resolve the environment the way [`chant lifecycle diff --live`](/chant/cli/lifecycle/) does.
...
| Kubernetes | Typed API client `LIST` per kind (chant #1074) | `status`, `managedFields`, server metadata, server annotations |
...
To automate this across many resources as a reviewable PR, use the [`ReconcileOp`](/chant/guide/reconciling-lifecycle/) workflow, which runs exactly this loop and opens the PR for you.

## See also

- [Reconciling Lifecycle](/chant/guide/reconciling-lifecycle/) — the reconcile and apply workflows
- [Lifecycle Models](/chant/concepts/lifecycle-models/) — where live import sits on the dial
