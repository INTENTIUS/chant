...
chant build --param tier=production  # Bind a declared build-time parameter
```

See the [build CLI reference](/chant/cli/build/) for full flag documentation, and [Build-Time Parameters](/chant/concepts/build-time-parameters/) for varying a build across environments without reading `process.env`.
...
`--watch` mode maintains this module graph and re-evaluates only the affected files when a source file changes.

<Diagram name="module-graph-watch" alt="Module graph showing forward import edges and reverse-walk re-evaluation edges when a file changes in watch mode" caption="Watch mode: reverse-walks the module graph to find affected files" />
