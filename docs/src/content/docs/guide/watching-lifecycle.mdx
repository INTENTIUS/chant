...
`receipts: [myReceipt, ...]` (typed `EffectReceipt` references) adds a read-only `Receipts` phase that reads each effect receipt through the receipt store and reports absent or differing receipts as findings, surfaced as a `StaleReceipts` search attribute — it never runs an effect and never writes a receipt.
...
`outcomeAttribute` is a generic Op feature, not a `WatchOp`-specific one — see [Outcome-based search attributes](/chant/guide/ops-reference/#outcome-based-search-attributes) for the full contract. The composite wires it like this:
