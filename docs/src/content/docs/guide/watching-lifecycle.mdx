...
- **`DRIFTED`** — observed in both snapshots; attributes changed. The deltas are listed inline. Decide whether to re-apply (snap back to declared) or update source (accept the new shape). A property-level drift that fires every tick because something else legitimately operates that field, an autoscaler on `replicas`, say, or a controller stamping an annotation on every sync, is worth declaring [`heldElsewhere()`](/chant/concepts/drift-detection/#heldelsewhere-a-field-declared-once-then-handed-over) rather than re-triaging on every run. It stops appearing as drift, and shows up in the plan's HELD section with who holds it and why.
