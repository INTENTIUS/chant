...
| `source` | Where the work came from: a gap, an issue (`issue`), the workspace (`kind: workspace` with a `member`), or a person's ask (`ask`). A gap may name the `finding-triage` answer that made it work as `answer`. |
...
A person's ask can become work with no gap or issue behind it, the way hud's sidebar and the kit's ask flow take one up (chaff#39, [#2851](https://github.com/INTENTIUS/chant/issues/2851)). `source.ask` names what they said and who said it:
...
`said` and `by` are required; the schema rejects an ask with neither. `at`, `via` (`hud`, `shell` or `mcp`) and `session` are optional, and name when the ask came in, where from, and the session it came up in. An item built from an ask opens `proposed` too, the [same first state](#proposed-work) any other suggestion does.
