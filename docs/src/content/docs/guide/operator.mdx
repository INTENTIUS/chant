...
writes the counterpart fact — a `GateResolutionRecord` at `_gates/<op>.jsonl` — or a merged PR resolves it just as validly (record the PR's URL in `--note` for a single audit trail either way).
