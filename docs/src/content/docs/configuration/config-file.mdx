...
`--env <name>` selects one and defaults to `local` when omitted. The selected environment is what [`chant lifecycle`](/chant/cli/lifecycle/) diffs against, the `env` identity stamped into the [ownership](#ownership) marker, and the key the [component release ledger](/chant/components/observability/) records each deploy under — as in `chant run --components <name> --env <env>` and `chant components status <env>`.
