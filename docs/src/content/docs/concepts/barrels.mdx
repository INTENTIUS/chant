...
`_.$.dataBucket.arn` resolves to a symbolic cross-resource reference in the evaluated output.
...
5. **Resolve property access.** When the evaluator encounters `$.dataBucket.arn`, it looks up `dataBucket` in the symbol table, finds its resource type, and resolves `arn` through the lexicon's attribute registry.
