...
**chant:** by default, chant imports your TypeScript and reads the resource objects it exports. Your constructors run. Synthesis does nothing else. No API calls, no state, no deployment. A separate evaluability lint pass (EVL) restricts source to literals, constants, and cross-resource references, so the executed result is equivalent to static data. Same conforming source always produces the same output. An opt-in build path (`chant build --fold`) goes one step further and skips execution entirely for source that stays inside that same subset, reducing it directly from the AST — see [Folded vs Run](/chant/concepts/typescript-as-data/#folded-vs-run).
...
- **Lexicon ecosystem is young, but covering.** Ten lexicons ship today (AWS CloudFormation, Azure ARM, GCP Config Connector, Kubernetes, Helm, Docker, GitHub Actions, GitLab CI, Forgejo Actions, and Temporal — Forgejo is a thin GitHub-Actions dialect). The architecture supports any spec-based operational domain — see [Lexicons](/chant/lexicons/overview/) for the current set and [Lexicon Authoring](/chant/lexicon-authoring/overview/) for adding new ones.
