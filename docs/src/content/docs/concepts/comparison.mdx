...
|---|---|---|---|---|
| **Language** | HCL (custom DSL) | TypeScript, Python, Java, Go, C# (executed) | TypeScript, Python, Go, C#, Java, YAML (executed) | TypeScript (static subset) |
...
| **Lifecycle scope** | Full | Full (via CloudFormation) | Full | Deterministic core + chosen lifecycle (observe / reconcile / apply) |
...
| **Drift detection** | Built-in (`plan` / `refresh`) | Via CloudFormation drift detection | Built-in (`refresh` / `preview`) | Built-in (`lifecycle diff --live` against any of 5 cloud lexicons + 4 artifact lexicons) |
