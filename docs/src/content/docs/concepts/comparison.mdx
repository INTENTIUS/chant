...
| **Drift detection** | Built-in (`plan` / `refresh`) | Via CloudFormation drift detection | Built-in (`refresh` / `preview`) | Built-in (`state diff --live` against any of 5 cloud lexicons + 4 artifact lexicons) |
...
The most consequential design difference after execution model is the state model.
...
- **Lexicon ecosystem is young, but covering.** Eleven lexicons ship today (AWS CloudFormation, Azure ARM, GCP Config Connector, Kubernetes, Helm, Docker, GitHub Actions, GitLab CI, Flyway, Slurm, Temporal). The architecture supports any spec-based operational domain — see [Lexicons](/chant/lexicons/overview/) for the current set and [Lexicon Authoring](/chant/lexicon-authoring/overview/) for adding new ones.
