...
Your own functions, arrow functions, and method calls (`naming.name(...)`) are never evaluated statically — the value would depend on running code. The only calls that fold are chant's own [registered authoring helpers](#registered-authoring-helpers). Where you need a computed value in foldable source, move the computation into a composite, which chant expands for real.
...
As of this measurement, **24 of 98** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **74** have at least one file that falls back to running.
