...
A function call is not a value chant can evaluate statically, with one closed exception: chant's own authoring helpers. `phase()`, `gate()`, `activity()`, `stackOutput()` and `output()` are pure functions of their arguments, and they are the API chant documents for writing components and outputs — source using them cannot avoid a call and still use the feature. The [converge rule](/chant/guide/converging-lifecycle/) builders are registered on the same grounds. `when()` and the predicate and action builders it composes each return a plain record built from their arguments, and a `ConvergeOp` rule table cannot be written without them.
...
### Method calls on folded values

Since [#1966](https://github.com/INTENTIUS/chant/issues/1966), a method call folds when its receiver itself folds to a real value, not one of fold's own symbolic placeholders, and the named property on that value is a function. The method name is not checked against any list; calling it is no different from what running the file would do, because the receiver is the same real object either way.
...
export const store = new StorageType({
  name: [prefix, "data"].join("-"),
});
```

This covers an array literal's own methods (`[...].join(",")`), a method on a live cross-file reference (`github.actor.toString()`), and a method on the result of another fold (`matrix("os").toString()`) alike. Whatever produced the receiver, `fold()` only asks whether it is now a real value with that property. It does not cover a method through a *package's* namespace import (see [Registered intrinsic calls](#registered-intrinsic-calls)). It also does not make the method's own arguments any more foldable than they'd otherwise be: `list.map((n) => n.toUpperCase())` still refuses, since the arrow-function callback is a value fold rejects on its own, independent of `.map` being a method call.
...
A function from a package and a function used as a value are never evaluated statically — the value would depend on running code. Three exceptions fold. Two are closed allowlists of names, checked against what your file actually imported: chant's own [registered authoring helpers](#registered-authoring-helpers), and a [lexicon intrinsic whose call form its lexicon opted in](#registered-intrinsic-calls). The third is open but local: a call to a [function from one of your own project files](#project-local-function-calls) whose body is itself in this subset. EVL001 still flags the call above because the lint rule sees one file at a time and cannot know what `getName` does; the fold decides by reading the body.

A method call (`naming.name(...)`, `list.join(",")`) is not on this list. [It folds on its own terms](#method-calls-on-folded-values) whenever its receiver is a real value and the named property is a function. What EVL001 above still flags is a call whose callee resolves to nothing chant can evaluate, whichever shape it takes.

### An array method whose callback is a function value

```typescript
const list = ["a", "b"];

export const store = new StorageType({
  name: list.map((n) => n.toUpperCase()),  // the callback, not the method call, is the problem
});
```

`.map` itself is an ordinary method call on a real receiver, which is the shape [Method calls on folded values](#method-calls-on-folded-values) covers. Its callback argument is what fold rejects. An arrow function used as a value is never foldable, whether called or not. `list.join(",")` above folds instead, because its argument is a plain string, not a function.
...
### Unregistered tagged template intrinsics

```typescript
// Rejected at fold time: unregistered tagged template intrinsic: unknownTag`...`
export const store = new StorageType({
  name: unknownTag`${prefix}-data`,
});
```

Not an EVL lint error. EVL has no intrinsic registry at lint time, so it treats every tagged template's interior as opaque regardless of registration (see [Intrinsic tagged templates](#intrinsic-tagged-templates)), and this passes lint. It fails only when `chant build` folds the file and resolves the tag against the intrinsics the loaded lexicons actually registered. A plain, untagged template, `` `${prefix}-data` `` with no tag at all, folds like any other value in the subset; see [Supported expressions](/chant/architecture/evaluator/#supported-expressions-fold-subset). The tag is what's unsupported here; template interpolation itself works.
...
What still forces a fallback to run, per file: **a function call used as a value that is neither a registered authoring helper, an opted-in intrinsic, nor a project-local function with a foldable body** — a package's function, an intrinsic whose lexicon has not opted it in, a helper of your own whose body reads the environment or does I/O. [Method calls](#method-calls-on-folded-values) are not automatically among them. Since #1966 a method call folds when its receiver is a real value and the named property is a function, so what actually blocks `list.map((n) => ...)` is its callback being a function used as a value, the same rule that blocks a bare function-as-value anywhere else, and not `.map` being a method call. One still falls back when its *receiver* doesn't resolve to a real value, such as `aws.Ref(...)` through a lexicon's namespace import (see [Registered intrinsic calls](#registered-intrinsic-calls)). Also: a composite call nested as a *value* inside something else rather than being the file's own top-level export, a `new ns.Type(...)` through a namespace import (the class has to be reachable through a named import to be constructed), a re-export, `export default`, `let`/`var`, and an exported class declaration.

A read whose object is `null` or `undefined` forces the same fallback (#2328). Running `cfg.nett.vpcId` where `cfg` has no `nett` key raises `TypeError: Cannot read properties of undefined`. Folding it to `undefined` would drop the property and let the build carry on, so the two paths would disagree about what the file means. Fold refuses it instead and the file runs, which puts the TypeError back on the line that caused it. The exception is optional chaining. `cfg.net?.vpcId` is defined to be `undefined` on a nullish object, and folds that way here too.
...
5. Every expression in it is in the ordinary [supported subset](#supported-patterns), extended with the two things a factory body exists for: `new Type(...)` in any value position (a member, a nested property object, an array element), and a call through a bare identifier (a nested composite, a registered helper, an opted-in intrinsic). A method call (`naming.name(...)`, `.map(...)`) is a property-access callee and stays out. This is narrower than [the top-level fold path](#method-calls-on-folded-values). The admissibility check here (`checkFactoryExpression` in `fold-import.ts`) runs before anything is resolved, so it has no receiver value to test, and it rejects every property-access callee on sight instead of deciding case by case.
...
As of this measurement, **96 of 109** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **13** have at least one file that falls back to running.
...
Folded files execute none of their own code, but a run-fallback file does — in the CLI's own process by default. `chant build --sandbox` (#1045) runs every run-fallback file for a build together, isolated, in one child process with the filesystem, process-spawn, and environment access locked down, instead of trusting the file to behave. It composes with folding, leaving folded files alone (there is nothing to isolate) and sandboxing the run-fallback remainder — see [Sandboxed Execution](/chant/architecture/sandbox/) for exactly what that does and does not protect against.

### Fold depth bounds

Three counters cap the recursion the fold path can do instead of letting it run away. Here is what the specification's `F-Depth` judgment says about them (J4 in `spec/judgments.md`, [INTENTIUS/typescript-as-data#46](https://github.com/INTENTIUS/typescript-as-data/issues/46)).

{/* rule: F-Depth */}
> An implementation bounds three recursions and may choose the values.

The rule also requires the values to be stated, and requires that exhausting one produce a fallback to the run path rather than a failure or a silent change of evaluation mode. The table below is chant's statement of its three.

Whether the call-depth bound is genuinely the implementation's to choose is not settled. `F-Eval-CallLocal` step 2 fixes it at 32 in its own text while `F-Depth` leaves it open, and [INTENTIUS/typescript-as-data#71](https://github.com/INTENTIUS/typescript-as-data/issues/71) is where the two get reconciled. What holds under either reading, and what chant meets, is that exhaustion falls back rather than producing wrong output.

| Bound | Value | Guards | Where |
|---|---|---|---|
| `MAX_FUNCTION_CALL_DEPTH` | 32 | nested [project-local function calls](#project-local-function-calls) | `packages/core/src/fold/fold.ts` |
| `MAX_INTERPRETATION_DEPTH` | 16 | nested [interpretable composite factory](#interpretable-composite-factories) bodies | `packages/core/src/discovery/fold-import.ts` |
| `MAX_RESOLUTION_DEPTH` | 200 | the cross-file [resolution stack](#cross-file-resource-references) | `packages/core/src/discovery/fold-import.ts` |

`MAX_FUNCTION_CALL_DEPTH` and `MAX_RESOLUTION_DEPTH` exhaustion both name themselves. A project-local call past 32 deep throws from inside `fold()` with a reason naming the bound (`call depth exceeded — is it recursive?`), forcing the calling file to run, exactly like the `[fold:run]` example under [Project-local function calls](#project-local-function-calls) above. A cross-file resolution past 200 deep is caught at the same stack that also detects an import cycle, with its own pointed reason (`cross-file resolution depth exceeded 200 (...) — this is almost certainly a resolution bug (memoization not taking effect), not a genuinely thousand-file-deep import chain`). This forces the target file to run and, per [Folded vs Run](#folded-vs-run) above, cascades to any file that folded while depending on it.

`MAX_INTERPRETATION_DEPTH` guards a composite whose factory body calls itself, directly or through a member, entirely within one module. That is a shape no cross-file stack can see, so a separate counter exists for it. Past 16 levels chant stops trying to interpret and the file falls back to run with a reason naming the bound, the same as the other two. It did not always. Until chant #2370 it declined to interpret and invoked the composite for real instead, which still folds the file, so the bound changed how the estate was evaluated while reporting it as folded, and nothing said so. That was the one place `F-Depth`'s second half was unmet, since giving up is not the same fact as a body sitting outside the [interpretable subset](#interpretable-composite-factories), and the two were sharing one answer. A composite genuinely nested this deep is not expected to occur. Sixteen is, in the implementation's own words, "deliberately small: real composite nesting is 2-3 deep, and a chain past this is a bug, not a design."
