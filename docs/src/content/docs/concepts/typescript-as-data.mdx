...
`MAX_INTERPRETATION_DEPTH` guards a composite whose factory body calls itself, directly or through a member, entirely within one module. That is a shape no cross-file stack can see, so a separate counter exists for it. Past 16 levels, chant stops trying to *interpret* that composite and invokes it for real instead, exactly as it would for any composite whose body sits outside the [interpretable subset](#interpretable-composite-factories); the file still folds, since invoking is the other of the two ways a composite call folds, not a `[fold:run]` fallback. That makes it silent in a way the other two aren't. Nothing today names the bound when it's hit, because the function enforcing it is documented to only ever decline, never fail. A composite genuinely nested this deep is not expected to occur; sixteen is, in the implementation's own words, "deliberately small: real composite nesting is 2-3 deep, and a chain past this is a bug, not a design."
