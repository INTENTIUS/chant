...
As of this measurement, **32 of 98** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **66** have at least one file that falls back to running.
...
As measured on the corpus after #1044 (chant `1fb1b375`, per-file fallback reasons classified by cause): a function call used as a value blocks 37 files and is the only blocker in 9 entries — down from 77 files and 21 entries before opting the AWS and Azure intrinsics in. What's left there is a lexicon that hasn't been audited yet (github's `Checkout`, 6 entries; helm's `include`/`printf`, 3) plus array `.map(...)` and project-local helpers, which are arbitrary JavaScript and stay rejected by construction. Cross-file references into a lexicon package's own exports (rather than a project's own source files) now block 116 files across 36 entries, tracked as [#1063](https://github.com/INTENTIUS/chant/issues/1063), and are the largest remaining cause.

Per lexicon, so the two can be judged separately: the AWS corpus went from 4 of 18 entries folding to **12 of 18**, and the 6 that remain are blocked by cross-file references, shape gaps, and composite-factory calls — not by intrinsics. The Azure corpus went from **0 of 13 to 0 of 13**: `Concat(...)` stopped blocking both entries where it was the only blocker, and both then stopped on `Azure.ResourceGroupLocation` used as a plain value, which is #1063. Azure's fold coverage is capped at zero by that, not by anything #1044 could have fixed. This is also the correction to what #1039 predicted: the intrinsics wiring was called "the single biggest lever" and moved one entry; the call form moved eight; and for Azure specifically the lever that matters is a different issue entirely.
