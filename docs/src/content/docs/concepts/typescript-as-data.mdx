...
As of this measurement, **95 of 108** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **13** have at least one file that falls back to running.
...
Three counters cap the recursion the fold path can do instead of letting it run away, each an implementation choice the specification's `F-Depth` judgment permits ([INTENTIUS/typescript-as-data#46](https://github.com/INTENTIUS/typescript-as-data/issues/46), J4 in `spec/judgments.md`). An implementation may pick its own bound, provided it states the bound and falls back rather than fails outright.
