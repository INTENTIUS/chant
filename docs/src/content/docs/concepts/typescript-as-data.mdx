...
What still forces a fallback to run, per file: **a function call used as a value that is neither a registered authoring helper nor an opted-in intrinsic** — your own helper, an array `.map(...)`, a method call, an intrinsic whose lexicon has not opted it in. Also: a composite call nested as a *value* inside something else rather than being the file's own top-level export, a `new ns.Type(...)` through a namespace import (the class has to be reachable through a named import to be constructed), a re-export, `export default`, `let`/`var`, and an exported function or class declaration.
...
As of this measurement, **76 of 101** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **25** have at least one file that falls back to running.
