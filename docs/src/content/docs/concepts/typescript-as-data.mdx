...
Folding is the **default build path** (#1134; opt-in via `--fold` until then): instead of importing and running a file, chant reduces its AST directly to the resource spec, with zero module execution. Nothing changes about what you write to use it — the supported subset above is the same subset that folds. What changes is how the build gets there:
...
| Default state | Off — pass `--no-fold` or set `build.fold: false` | **On by default** (#1134) |
...
As of this measurement, **55 of 101** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **46** have at least one file that falls back to running.
