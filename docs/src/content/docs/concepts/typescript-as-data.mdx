...
As of this measurement, **99 of 112** example projects in the corpus fold completely — every file in them reduces to a value with zero module execution. The remaining **13** have at least one file that falls back to running.
...
Three counters cap the recursion the fold path can do instead of letting it run away. Here is what the specification's `F-Depth` judgment says about them (J4 in `spec/judgments.md`).
...
The values are chant's to choose. `F-Eval-CallLocal` step 2 says so outright: "The value is the implementation's; exhaustion is a fallback and never wrong output". The numbers below are chant's stated choice, which the rule requires it to make.
