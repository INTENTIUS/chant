...
Run `chant lint`, or rely on the editor, to catch type errors and patterns that are valid TypeScript but not statically evaluable. See [TypeScript as Data](/chant/concepts/typescript-as-data/) for the full list. Staying inside that subset is what lets a file skip Stage 4's import/execution entirely on the default fold path, and it is what guarantees the imported source is safe to run when a file falls back; see [Folded vs Run](/chant/concepts/typescript-as-data/#folded-vs-run).
...
## Stage 4: Fold or Collect Resources

By default chant reduces each discovered file's AST directly to its resource objects, with no module execution. A file that steps outside the fold subset is instead imported with the TypeScript runtime and its exported resource objects collected, which is also what `--no-fold` forces for every file. This stage describes that run path; see [Folded vs Run](/chant/concepts/typescript-as-data/#folded-vs-run) for which files take which.
