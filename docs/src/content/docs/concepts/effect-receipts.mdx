...
chant Ops run exactly these effects today: `shell` steps, migrations, pipeline triggers. A run's own ledger record remembers them *within* a run, but nothing remembered across runs. The [release ledger](/chant/components/observability/#the-release-ledger) records what deployed but never gates anything. So before receipts, every Op run re-ran every effect, or teams hand-rolled guards in scripts, and nothing at plan time could say "this apply will fire db-migration."
