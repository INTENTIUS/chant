...
The lifecycle spans declarative deployment workflows ([`Op`](/chant/guide/ops/) — phased convergent workflows declared in `*.op.ts` files, run via `chant run`), continuous observation ([`WatchOp`](/chant/guide/watching-lifecycle/) — periodic snapshot+diff on the Op's own `schedule`), the observational lifecycle model ([`chant lifecycle`](/chant/cli/lifecycle/) — non-authoritative snapshots; see [Drift Detection](/chant/concepts/drift-detection/)), a release model ([Components](/chant/concepts/components/) — releasable units composed from a bounded capability set and run by a generated Op), and agentic skills for import and drift detection. They share a principle: the deterministic core provides context and structure that the lifecycle layers, agents, and operators act on with the same auditability underneath.
