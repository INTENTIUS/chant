...
The design supports declarative deployment workflows ([`Op`](/chant/guide/ops/) — phased Temporal workflows declared in `*.op.ts` files, run via `chant run`), continuous state observation ([`WatchOp`](/chant/guide/watching-state/) — periodic snapshot+diff scheduled via Temporal), observational state ([`chant state`](/chant/cli/state/) — non-authoritative snapshots; see [Drift Detection](/chant/concepts/drift-detection/) for the model), and agentic skills for import and drift detection. These extensions share a principle: chant provides context and structure, agents and operators do the operational work.
