...
The design supports declarative deployment workflows ([`Op`](/chant/guide/ops/) — phased Temporal workflows declared in `*.op.ts` files, run via `chant run`), continuous state observation ([`WatchOp`](/chant/guide/ops/#continuous-observation) — periodic snapshot+diff scheduled via Temporal), observational state (non-authoritative snapshots of deployment metadata via [`chant state`](/chant/cli/state/)), and agentic skills for import and drift detection. These extensions share a principle: chant provides context and structure, agents and operators do the operational work.
