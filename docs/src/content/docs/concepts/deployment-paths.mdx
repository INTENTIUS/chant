...
description: Hand off to your own pipeline, Ops, Components, or raw Temporal — pick the right fit for your project
...
<Diagram name="deployment-paths" alt="Decision tree: existing deploy pipeline -> hand off; need observability, gates, or rollback -> Ops or Raw Temporal" caption="Choosing your deployment model" />
...
**How to get started:** [Components Overview](/chant/components/overview/), then [Component Contract](/chant/components/component-contract/). Runs on the local executor by default; only gated or crash-resumable components need Temporal. For where this sits against Ops and the lifecycle dial, see [Components](/chant/concepts/components/).
