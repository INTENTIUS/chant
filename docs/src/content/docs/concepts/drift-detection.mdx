...
Depth is opt-in per lexicon and changes nothing for the others — see the [coverage matrix](/chant/lexicons/overview/#runtime-observation-coverage) for who reports it, and [`--update-baseline`](/chant/cli/lifecycle/#--update-baseline--accept-a-deviation) for accepting a deviation.
