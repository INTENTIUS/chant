...
When a whole environment is done rather than one resource, [`chant lifecycle teardown <env>`](/chant/cli/lifecycle/#lifecycle-teardown-env) deletes everything carrying that environment's marker in one marker-scoped pass — also available as the [`envTeardown` Op step](/chant/guide/ops-reference/#step-builders) for a run behind a human gate.
