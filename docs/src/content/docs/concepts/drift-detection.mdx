...
`chant state snapshot <env>` writes a record of what was observed in the cloud at a point in time. It's stored on a `chant/state` orphan branch in your repo. There's no central state server, no lock file, no encrypted backend.
...
`chant state diff <env> --live` returns ten categories — six for resources, four for artifacts.
...
Same lexicon may emit both — for instance a future K8s lexicon could report Deployments as resources *and* in-cluster Pods (created by the Deployment, not by chant) as artifacts. `state diff --live` shows them in separate sections.
...
- [`chant state`](/chant/cli/state/) — CLI reference for snapshot, show, diff, log
- [Watching State](/chant/guide/watching-state/) — turn `state diff --live` into a recurring Temporal workflow
