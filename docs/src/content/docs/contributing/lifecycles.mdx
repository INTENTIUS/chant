...
chant's core is a compiler: pure, deterministic, lint-gated TypeScript-to-spec compilation. Everything past synthesis — state, apply, drift, reconcile, retire — is the **lifecycle**, a first-class layer you build on that core rather than a fixed part of it. The shape of the project is:
...
The first door ships today. A lifecycle is just orchestration over chant's primitives, and you declare orchestration as [Ops](/chant/guide/ops/) in `*.op.ts` files — named, phased workflows that run on the local executor or, when they need a machine of their own, on a fountain steward.
...
- [`ApplyOp`](/chant/guide/reconciling-lifecycle/) — `code → cloud`: apply through the target's own delete path, owned-only on every target ([what bounds each one](/chant/guide/ops-reference/#delete-paths)).
...
Package a reusable one as a composite that returns an `Op` carrying its own `schedule`, exactly as the built-ins do — see `packages/core/src/op/composites/` for reference. That is a complete, shippable lifecycle.
...
- **A runtime boundary** — `chant run --on <lexicon>` is a clean interface between *declaring* a workflow and choosing what hosts it.
...
The two doors close the gap BYOL could otherwise open. "Bring your own lifecycle" must never read as "you're on your own": Door 1 gives you rails to build on today (chant runs the lifecycle you compose), and Door 2 means an existing investment isn't thrown away — it's plugged in. Own the socket, outlast the lifecycle war.

## See also

- [`lifecycle-reconcile-aws` example](https://github.com/INTENTIUS/chant/tree/main/lexicons/aws/examples/lifecycle-reconcile-aws) — a Door 1 lifecycle (ReconcileOp + ApplyOp) wired to a real stack
...
- [Ops as teammates](/chant/concepts/durable-workflows/) — when a lifecycle wants a hosted runtime
