...
- [`ApplyOp`](/chant/guide/reconciling-lifecycle/) — `code → cloud`: apply through the target's own delete path, owned-only on the `kubectl` and `cloudformation` targets ([the scope differs per target](/chant/guide/ops/#apply-code--cloud)).
