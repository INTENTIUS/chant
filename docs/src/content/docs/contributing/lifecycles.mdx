...
- [`ApplyOp`](/chant/guide/reconciling-lifecycle/) — `code → cloud`: apply through the target's own delete path, owned-only on every target ([what bounds each one](/chant/guide/ops/#apply-code--cloud)).
