...
```typescript
// chant-disable COMP004 -- the release manager approves this rollout
```
...
```typescript
// ❌ Triggers COMP004 — no acknowledgment that this component stops for a person
deploy: [
  phase("Node 1", [
...
**Fix:** if the component is meant to stop for a person, suppress with `// chant-disable COMP004 -- <reason>` to document who approves it and why. There is no per-component runtime field — the same declaration runs wherever it is invoked, so this rule always flags a `gate` unless acknowledged.
