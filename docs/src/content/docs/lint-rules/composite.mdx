...
Object literals and arrays-of-objects inside a Composite factory that don't reference `props`, sibling members, or barrel refs (`_.$.name`) are extractable constants. Move them to a separate file (e.g. `defaults.ts`) and reference via the barrel.
...
```typescript
// ✅ Fixed — extract to defaults.ts, reference via barrel
const LambdaApi = Composite<Props>((props) => {
  const role = new _.Role({
