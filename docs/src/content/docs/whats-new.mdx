...
| [#124](https://github.com/INTENTIUS/chant/issues/124) | [`ApplyOp`](/chant/guide/reconciling-lifecycle/) composite — the `code → cloud` workflow via native apply (`kubectl apply` / CloudFormation deploy / ARM); deletes ride the target's own delete path ([what bounds each one](/chant/guide/ops/#apply-code--cloud)) |
