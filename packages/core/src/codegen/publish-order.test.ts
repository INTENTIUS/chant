...
/** The order publish-packages.sh would actually use, asked of the script itself. */
function publishOrder(): string[] {
  const script = readFileSync(join(REPO, "scripts/publish-packages.sh"), "utf8");
  const fn = script.match(/^publishable_dirs\(\) \{$.*?^\}$/ms);
  if (!fn) throw new Error("publishable_dirs() not found in scripts/publish-packages.sh");
