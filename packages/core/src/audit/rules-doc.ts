...
const GROUPS: Array<{ heading: string; prefixes: string[]; blurb: string }> = [
  ...
  { heading: "Terraform (TF)", prefixes: ["TF"], blurb: "Run against Terraform root modules (a directory of `.tf` files) parsed into the entity graph, so the same rules fire on `chant build` over `terraform.roots` and on `chant audit` (#2083)." },
