/**
 * chant#2591 — `init --force`, `dev onboard` and `import --agents` learn the
 * lexicons a project declares by path without running its `chant.config.ts`.
 *
...
beforeEach(() => {
  ...
  writeFileSync(join(project, "gh", "index.ts"), "export {};\n");
  writeFileSync(
