 ...
 *
 * It reads the workspace declaration for a brokered key, so it loads code
 * under `workspace/`: the `decide` activity imports it on first call, never
 * at module load (#2525 rule 5).
 */

import type { ModelAsk, ModelRequest, WireAnswer } from "../workspace/points";
import { readDeclaration } from "../workspace/declaration";
import { locateWorkspace } from "../workspace/which-chant";
import type { BackendKey, BrokeredKey, DecideBackend } from "./decide-config";
...
export class DecideConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "DecideConfigError";
  }
}
 ...
 * reads it from. Throws a {@link DecideConfigError} when no member declares
...
export function brokeredCapability(key: BrokeredKey, cwd: string): BrokeredCapability {
  ...
  try {
    ...
    throw new DecideConfigError(`the key names the brokered capability ${key.capability}, and the workspace declaration can't be read: ${err instanceof Error ? err.message : String(err)}`);
  }
  ...
  if (declaring.length === 0) {
    ...
    throw new DecideConfigError(`${what} (box.capabilities in chant.workspace.json, #2726)`);
  }
  if (declaring.length > 1) {
    throw new DecideConfigError(`the capability ${key.capability} is declared by ${declaring.map((m) => m.name).join(" and ")}: name the member in the key`);
  }
  const member = declaring[0];
  const capability = member.box!.capabilities.find((c) => c.name === key.capability)!;
  if (capability.broker === null) {
    throw new DecideConfigError(`member ${member.name}'s capability ${key.capability} names no broker (WSP122), so nothing holds its key`);
  }
...
export function checkKey(key: unknown, where: string, cwd: string): void {
  ...
  if (typeof key === "string") {
    throw new DecideConfigError(`${where} is a literal string; a key is { env: "<VARIABLE>" } or a brokered capability, never the key itself (SYS001)`);
  }
...
export async function postQuestion(backend: DecideBackend, name: string, request: ModelRequest, opts: Omit<BackendAskOptions, "backends">): Promise<{ model: string; answer: WireAnswer; response: BackendResponse }> {
  ...
  const r = parsed as Partial<BackendResponse> | null;
  ...
  return { model: r.model, answer, response: r as BackendResponse };
}
 ...
 */
export function backendAsk(opts: BackendAskOptions): ModelAsk {
  return async (request) => {
    const backend = opts.backends[request.backend];
    if (!backend) throw new Error(`no backend named ${request.backend} is configured (decide.backends)`);
