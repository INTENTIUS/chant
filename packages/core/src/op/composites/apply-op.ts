 ...
 *
 * A gate is a fact, not a wait (#2119): the local executor reads the gate
 * ledger, and a run that finds no resolution ends `gated` with the `chant
 ...
 *
 * // gated destructive apply — `chant approve prod-apply approve-apply`, then re-run
