/**
 * A stub `POST /v1/systemone` server for tests of the `decide` activity
 * (`@intentius/chant/op/__fixtures__/decide-stub-backend`): a real HTTP server on
 ...
 * // decide.backends, or a decide step's backends: { systemone: { url: stub.url, key: { env: "STUB_KEY" } } }
 * await stub.close();
 * ```
 *
 ...
 *
 * It lives under `__fixtures__` because it listens on a port, and no module
 * chant runs does (#2657, `test/no-listener.test.ts`): only tests start it.
 */

import { createServer, type IncomingMessage, type ServerResponse } from "node:http";
import type { AddressInfo } from "node:net";
import type { WireAnswer } from "../../workspace/points";
import { SYSTEMONE_PATH, type BackendRequest } from "../decide-backend";
...
export interface StubBackendOptions {
  ...
  answers?: Record<string, WireAnswer> | ((question: StubQuestion, request: BackendRequest) => WireAnswer);
...
export async function startStubBackend(opts: StubBackendOptions = {}): Promise<StubBackend> {
  ...
  const server = createServer((req, res) => {
    void (async () => {
      ...
      const request = body as BackendRequest;
