 ...
 * answer. Core's own since #2828, next to the decision points it asks; the
 * typed step builder is `decide` in `../builders.ts`.
 *
 ...
 * contract (`../decide-read-inputs.ts`), the call to a `POST /v1/systemone`
 * backend (`../decide-backend.ts`), and refusing a misconfigured backend before
 * anything is asked or written. The backends are the step's own `backends`, or
 * `decide.backends` in chant.config (`../decide-config.ts`).
 *
 ...
 *
 * The workspace code, the client and the read-contract reads load on first
 * call, so importing core's activities loads nothing under `workspace/`
 * (#2525 rule 5).
 */

import { loadChantConfig } from "../../config";
import { askPointInRun, brokeredModelAsk, isPointWait, type BrokeredModelAsk } from "../steward-points";
import { currentStewardTurn } from "../steward-turn";
import type { QuestionView } from "../../workspace/decide";
import type { PointView } from "../../workspace/points-cli";
import type { Escalation } from "../../workspace/points";
import { backendSchema, type DecideBackend } from "../decide-config";
import type { BackendAskOptions, BackendResponse } from "../decide-backend";
...
export interface DecideArgs {
  ...
  cwd?: string;
  /** Backends by name, in place of `decide.backends` in chant.config. */
  backends?: Record<string, DecideBackend>;
...
export interface DecideDeps {
  ...
  transport?: BackendAskOptions["transport"];
  env?: NodeJS.ProcessEnv;
  /** The date written as asked_on, YYYY-MM-DD. */
  on?: string;
  onResponse?: (backend: string, response: BackendResponse) => void;
}
...
/** The backends the args give, or `decide.backends` from the chant.config in `cwd`. Each is checked; a literal key is refused. */
async function backendsFor(args: DecideArgs, cwd: string): Promise<Record<string, DecideBackend>> {
  const { checkKey, DecideConfigError } = await import("../decide-backend");
  let raw: unknown = args.backends;
  if (raw === undefined) {
    const { config } = await loadChantConfig(cwd);
    raw = config.decide?.backends ?? {};
  }
  if (raw === null || typeof raw !== "object" || Array.isArray(raw)) throw new DecideConfigError("backends must be an object of backend name to { url, key? }");
  const out: Record<string, DecideBackend> = {};
  for (const [name, b] of Object.entries(raw as Record<string, unknown>)) {
    ...
    if (!parsed.success) throw new DecideConfigError(`backend ${name} is not { url, key?, timeoutMs? }: ${parsed.error.issues.map((i) => `${i.path.join(".") || "(backend)"} ${i.message}`).join("; ")}`);
...
export async function runDecide(args: DecideArgs, deps: DecideDeps = {}, signal?: AbortSignal): Promise<DecideResult> {
  ...
  const { backendAsk, brokeredCapability, checkKey, DecideConfigError } = await import("../decide-backend");
  const configError = <T>(f: () => T): T => {
    try {
      return f();
    } catch (err) {
      if (err instanceof DecideConfigError) throw new DecideError("backend-invalid", err.message);
      throw err;
    }
  };
  let backends: Record<string, DecideBackend>;
  try {
  ...
  for (const d of point.deciders) {
    ...
    if (!backend) {
      throw new DecideError("backend-invalid", `${args.point}'s model decider names the backend ${d.backend}, and none is configured (decide.backends${Object.keys(backends).length ? `: ${Object.keys(backends).join(", ")}` : " is empty"})`);
    }
  ...
  const call = backendAsk({ backends, cwd, ...(deps.transport ? { transport: deps.transport } : {}), ...(deps.env ? { env: deps.env } : {}), ...(signal ? { signal } : {}), ...(deps.onResponse ? { onResponse: deps.onResponse } : {}) });
  ...
  if (args.dryRun) {
    ...
    const doc = await askPoint({
      ...
      ...(turn ? { steward: { name: turn.steward, ...(turn.run ? { run: turn.run } : {}) } } : { client: { name: "chant decide" } }),
      dryRun: true,
    });
