...
import { enterStewardTurn, isPointWait, resetStewardTurn, type WaitingPoint } from "..";
import { loadActivities } from "../activity-registry";
import { answerPoint, type QuestionView } from "../../workspace/decide";
import { workspacePoints } from "../../workspace/points-cli";
import type { WireAnswer } from "../../workspace/points";
import { startStubBackend, type StubBackend } from "../__fixtures__/decide-stub-backend";
import { cleanScratch, MODEL, recordText, workspace } from "../../workspace/__fixtures__/decide-workspace";
...
describe("the key", () => {
  ...
  test("a backend the point names but nobody configured is refused", async () => {
    ...
    expect((err as DecideError).code).toBe("backend-invalid");
    expect((err as Error).message).toContain("names the backend systemone, and none is configured (decide.backends: other)");
  });
});
...
describe("the activity as an Op step runs it", () => {
  test("loadActivities() registers decide with no lexicon configured", async () => {
    const activities = await loadActivities([]);
    expect(typeof activities.get("decide")).toBe("function");
  });

  test("backends come from decide.backends in the project's chant.config", async () => {
    writeFileSync(join(root, "chant.config.json"), JSON.stringify({ decide: { backends: { systemone: { url: stub.url, key: { env: "STUB_KEY" } } } } }));
    try {
