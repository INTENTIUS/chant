...
import { backendAsk, postQuestion, questionName } from "./decide-backend";
import { requestProblem, startStubBackend, type StubBackend } from "./__fixtures__/decide-stub-backend";
...
describe("the wire-format client", () => {
  ...
  test("a name no backend has, and a timeout, throw", async () => {
    await expect(backendAsk({ backends: {}, cwd: process.cwd() })(request)).rejects.toThrow("no backend named local is configured");
