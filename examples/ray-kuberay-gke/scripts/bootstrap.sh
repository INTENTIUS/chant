#!/usr/bin/env bash
# bootstrap.sh — create the Config Connector management cluster for ray-kuberay-gke.
#
# Run this once per GCP project before deploying the example.
# Creates a single-node GKE cluster with the Config Connector addon and Workload
# Identity. Config Connector reconciles all GCP resources in config.yaml from
# this cluster.
#
# Usage:
...
echo "==> Creating GKE management cluster with Config Connector + Workload Identity..."
...
echo "==> Getting credentials for management cluster..."
...
echo "==> Granting Config Connector SA IAM roles..."
for role in roles/editor roles/iam.securityAdmin; do
...
echo "Next: export GCP_PROJECT_ID=${project_id} GCP_REGION=${region} && npm run build:gcp"
