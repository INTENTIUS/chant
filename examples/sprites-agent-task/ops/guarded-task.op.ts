 ...
 * failure, `spriteRestore` the labeled checkpoint instead of unwinding with an
 * inverse action. A fast VM checkpoint is the transactional boundary — the
 * environment itself is the transaction. `./risky.sh` corrupts the workspace and
 * exits non-zero, so the `Run` phase fails and the Op-level `onFailure` `Restore`
 * rewinds the sprite to `pre-run`.
 */
export default Op({
  name: "guarded-task",
