...
// Syncs certificates from GCP Secret Manager into K8s Secrets via Workload Identity.
...
export const nodeCertsSecret = new ExternalSecret({
  metadata: { name: "cockroachdb-node-certs-eso", namespace: "crdb-east" },
  spec: {
    refreshInterval: "1h",
...
export const clientCertsSecret = new ExternalSecret({
  metadata: { name: "cockroachdb-client-certs-eso", namespace: "crdb-east" },
  spec: {
    refreshInterval: "1h",
    secretStoreRef: { name: "gcp-secret-manager", kind: "ClusterSecretStore" },
