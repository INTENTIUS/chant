# AWS Shared ALB — Cross-Lexicon UI Service Example
...
- Target group + ALB listener rule for `/` and `/*` (priority 200)
...
1. **build** — `build-image`: builds Docker image with Docker-in-Docker, pushes to ECR (`alb-ui`), tags `:latest` on default branch
...
- [gitlab-aws-alb-api](../gitlab-aws-alb-api) — API Fargate service (cross-lexicon)
