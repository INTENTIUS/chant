/**
 * `app-10`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-b`.
 *
...
export const app10LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-10/${Ref(app10TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app10TableName) }];

export const app10Queue = new Queue({
  QueueName: "fan-out-estate-app-10",
