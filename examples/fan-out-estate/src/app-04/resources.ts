/**
 * `app-04`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-a`.
 *
...
export const app04LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-04/${Ref(app04TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app04TableName) }];

export const app04Queue = new Queue({
  QueueName: "fan-out-estate-app-04",
