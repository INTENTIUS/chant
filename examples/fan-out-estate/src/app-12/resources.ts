/**
 * `app-12`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-b`.
 *
...
export const app12LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-12/${Ref(app12TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app12TableName) }];

export const app12Queue = new Queue({
  QueueName: "fan-out-estate-app-12",
