/**
 * `app-09`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-b`.
 *
...
export const app09LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-09/${Ref(app09TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app09TableName) }];

export const app09Queue = new Queue({
  QueueName: "fan-out-estate-app-09",
