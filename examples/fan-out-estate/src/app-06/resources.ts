/**
 * `app-06`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-a`.
 *
...
export const app06LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-06/${Ref(app06TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app06TableName) }];

export const app06Queue = new Queue({
  QueueName: "fan-out-estate-app-06",
