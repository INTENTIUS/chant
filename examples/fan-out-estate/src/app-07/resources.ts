/**
 * `app-07`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-b`.
 *
...
export const app07LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-07/${Ref(app07TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app07TableName) }];

export const app07Queue = new Queue({
  QueueName: "fan-out-estate-app-07",
