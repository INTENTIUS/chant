/**
 * `app-11`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-b`.
 *
...
export const app11LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-11/${Ref(app11TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app11TableName) }];

export const app11Queue = new Queue({
  QueueName: "fan-out-estate-app-11",
