/**
 * `app-03`: one CloudWatch log group and one SQS queue, sitting on
 * `cluster-a`.
 *
...
export const app03LogGroup = new LogGroup({
  LogGroupName: Sub`/chant/fan-out-estate/app-03/${Ref(app03TableName)}`,
  RetentionInDays: 14,
  Tags: tags,
});
...
const queueTags = [...tags, { Key: "chant:table", Value: Ref(app03TableName) }];

export const app03Queue = new Queue({
  QueueName: "fan-out-estate-app-03",
